#!/usr/bin/env python3
"""Fail-closed sequential driver for the frozen PREREG_EE_V3 family."""
...
def frozen_sources_ok() -> None:
    for rel in FROZEN_FILES:
        ...
        if tracked.returncode != 0 or clean.returncode != 0:
            raise RuntimeError(f"E-E V3 source is not committed/frozen at HEAD: {rel}")
...
def exact_private_names_ok() -> None:
    allowed = {"input", common.READY.name}
    ...
    if unexpected:
        raise RuntimeError(f"unexpected E-E V3 private artifacts: {unexpected}")
...
def preflight(*, new_campaign: bool) -> None:
    ...
    if common.ADJUDICATION.exists():
        if new_campaign:
            raise RuntimeError("E-E V3 public adjudication exists without campaign status")
        prior_status = common.load_json(common.STATUS)
        if (prior_status.get("state") not in {"endpoints_complete_adjudicating", "complete", "failed"}
                or counts_by_presence() != (16, 16)):
            raise RuntimeError("E-E V3 adjudication exists outside a completed endpoint family")
    if new_campaign:
        if common.STATUS.exists() or common.READY.exists():
            raise RuntimeError("new E-E V3 campaign has prior status/READY artifact")
        for arm, seed in common.expected_pairs():
...
def validate_resume_status() -> None:
    ...
    if (status.get("protocol") != common.PROTOCOL
        ...
        raise RuntimeError("E-E V3 resume status drift")
...
def main() -> int:
    ...
    if args.preflight_only:
        print("PREREG_EE_V3 committed-source preflight: PASS")
        return 0
    if not new_campaign:
        ...
        if common.ADJUDICATION.is_file():
            ...
            print("PREREG_EE_V3 CAMPAIGN ALREADY ADJUDICATED; STATUS FINALIZED")
            return 0
    trained, evaluated = counts_by_presence()
    ...
    try:
        for arm, seed in common.expected_pairs():
            ...
            if terminal.exists():
                ...
                run("run_ee_v3.py", "--arm", arm, "--seed", str(seed))
                common.validate_training_bundle(arm, seed)
            trained, evaluated = counts_by_presence()
            common.atomic_json(common.STATUS,
                               status_value("training", trained, evaluated, []))
        ...
        common.atomic_json(common.STATUS, status_value("evaluating", 16, evaluated, []))
        ...
        for arm, seed in common.expected_pairs():
            ...
            if any(present) and not all(present):
                raise RuntimeError(f"incomplete sealed TEST attempt; no rerun allowed: {arm}/{seed}")
            if not all(present):
                run("eval_ee_v3.py", "--arm", arm, "--seed", str(seed))
            trained, evaluated = counts_by_presence()
            common.atomic_json(common.STATUS,
                               status_value("evaluating", trained, evaluated, []))

        trained, evaluated = counts_by_presence()
        if (trained, evaluated) != (16, 16):
            raise RuntimeError(f"family completion count mismatch: {(trained, evaluated)}")
        common.atomic_json(common.STATUS,
                           status_value("endpoints_complete_adjudicating", 16, 16, []))
        run("adjudicate_ee_v3.py")
        if not common.ADJUDICATION.is_file():
            raise RuntimeError("adjudicator returned without a public adjudication")
        final = status_value("complete", 16, 16, [])
        final["adjudication_sha256"] = common.sha256(common.ADJUDICATION)
        common.atomic_json(common.STATUS, final)
        print("PREREG_EE_V3 CAMPAIGN COMPLETE AND ADJUDICATED")
        return 0
    except Exception as exc:
        trained, evaluated = counts_by_presence()
        common.atomic_json(common.STATUS,
                           status_value("failed", trained, evaluated, [str(exc)]))
        raise
...
if __name__ == "__main__":
    try:
        ...
        print(f"E-E V3 CAMPAIGN FAILURE: {exc}", file=sys.stderr, flush=True)
        raise
