...
| 2 | **V2 VOID; prospective gap remains open while V3 executes** | Every existing headline FIR estimate remains explicitly outcome-known/test-exposed, and no independent-confirmation claim is made. `Digital_Music` V1 ended `DM-V1-FEASIBILITY-VOID` when recursive five-core filtering produced an empty split. Software V2 was then frozen and launched from clean commit `c1048ba6`, but the later fixed-cutoff audit found a literal terminal contradiction before any endpoint existed: the protocol forbids TEST access before all 16 checkpoints, while both driver preflight and the imported trainer opened `Software.test.csv` for the transductive catalog. `--no-test-eval` suppressed scoring, not access. The campaign was stopped at one training JSON/two checkpoints/zero endpoint seals; no result or checkpoint content was read. `FIR_PROSPECTIVE_SW_V2_INTEGRITY.md` records permanent `SW-V2-INTEGRITY-FAIL`. New-seed Software V3 was frozen and pushed before launch at tag `fir-prospective-sw-v3-freeze` / commit `9c8f1a16`; it explicitly permits structural all-split catalog access while prohibiting TEST scoring until immutable READY, binds the exact runtime/input/dependency bytes and argv, uses exclusive attempt/evaluation seals, and requires its committed driver to invoke the committed adjudicator immediately after all 16 sealed endpoint pairs exist. The live campaign has no scientific verdict yet, remains local same-user custody rather than independent confirmation, and contributes no acceptance uplift unless and until its frozen adjudicator returns a valid result. |
...
| 4 | **CLOSED through pointwise; V3 protocol frozen, final evidence replay pending** | `bootstrap_public_clone.py` hydrates and raw-hash-verifies every release section, including the 144 control and 72 pointwise endpoint/sidecar/checkpoint files. A fresh HTTPS clone installed and verified the complete 354-asset / 8,274,248,666-byte pointwise boundary and passed exact HSTU parity, every adjudicator, Office V1 VOID retention, and the strict graph/manifest gate. The five outcome-free Software preparation assets (three splits, title cache, item map) are also public and manifest-verified, raising the live boundary to 359 assets / 8,375,684,035 bytes. The V3 preregistration, exact-runtime lock, driver, frozen trainer, sealed evaluator, and adjudicator are Git/release-manifest bound at `9c8f1a16`; a fresh remote clone passed the structural, byte-identity, environment, input-hash, catalog, and no-prior-output preflight before the immutable attempt was created. The final post-adjudication ritual must still upload every sealed V3 artifact and repeat the strict fresh-HTTPS replay over the enlarged boundary. |
