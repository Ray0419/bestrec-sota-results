# -*- coding: utf-8 -*-
"""Outcome-silent one-shot TEST evaluator for PREREG_FIR_POINTWISE_V1."""
...
def main():
    ...
    if out_path.exists() or users_path.exists() or seal_path.exists():
        raise SystemExit(
            "refusing repeat TEST access: final artifact or STARTED seal exists")

    base = json.load(open(run_path, encoding="utf-8"))
    cfg = base["config"]
    arm = cfg.get("fir_control")
    if arm not in ARMS or cfg.get("fir_control_kernel") != 16:
        raise SystemExit("run is outside frozen pointwise protocol arms")
    ...
    if base.get("best_test") is not None or any(
            "test" in x for x in base.get("history", [])):
    ...
    train, valid, test, _, items = rsp.reindex(
    ...
    emb_path = (Path(cache) if cache else rsp.EMB_CACHE_DIR /
                f"sbert_titles_{category}.npy")
    ...
    # The helper's constructor is parameterized through its module-global rsp.
    # Bind it to the protocol-frozen trainer rather than the evolving live file.
    fee.rsp = rsp
    ...
    with open(tmp_users, "wb") as f:
        np.savez_compressed(
            f, **{k: np.asarray(v) for k, v in records.items()})
