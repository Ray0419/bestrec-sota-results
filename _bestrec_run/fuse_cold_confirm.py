# -*- coding: utf-8 -*-
"""PREREG_COLDFUSE_V1 streaming confirmatory evaluator (E-G stage 2).

Streams batches (never materializes user x item matrices), so it runs on
every catalog incl. Office/CDs. VAL pass sweeps the FROZEN candidate grid
(profile x monotone frequency-binned text weights); the frozen VAL-only
rule selects one candidate; TEST evaluates reference + selected only.

Usage:
  python fuse_cold_confirm.py <base_run.json> [--no-ease] [--out o]
"""
...
def candidates():
    ...
    for prof in ("uniform", "exp0.9"):
        for t, m, h in itertools.product(WSET, WSET, WSET):
            if t >= m >= h:
                out.append({"profile": prof, "wt_tail": t, "wt_mid": m,
                            "wt_head": h})
    return out          # 2 x 20 = 40 (incl. two null triples)
...
def stream_eval(model, users, user_seqs, extra, user_times, xtimes, cfg,
                pad_id, n_items, E_gpu, B_gpu, we, systems, bin_gpu,
                tgt_bin, collect_per_user=False):
    """systems: list of dicts -- {"name", "profile"|None, "wvec"|None}.
    profile None => reference (no text term). Returns per-system metrics
    (overall + per-target-bin) and optional per-user ndcg arrays."""
    n = len(users)
    max_seq_len = cfg["max_seq_len"]
    use_times = getattr(model, "use_time_bias", False) and user_times is not None
    acc = {s["name"]: {"nd": 0.0, "hit": 0.0,
                       "nd_b": np.zeros(3), "n_b": np.zeros(3)}
           for s in systems}
    per_user = ({s["name"]: np.zeros(n, dtype=np.float32) for s in systems}
                if collect_per_user else None)
    need_prof = sorted({s["profile"] for s in systems if s["profile"]})
    tgt_all = np.asarray([0] * 0)
    t0 = time.time()
    with torch.no_grad():
        ...
        for s0 in range(0, n, BATCH):
            ...
            for prof in need_prof:
                ...
                zt[prof] = zrow(P @ E_gpu.T)
            # padded seen-index tensor for vectorized mask correction
            ...
            tgt = torch.tensor([tgt_bin["dict"][u] for u in bu],
                               dtype=torch.long, device=DEVICE)
            ar = torch.arange(Bn, device=DEVICE)
            tb = tgt_bin["bin"][tgt.cpu().numpy()]
            for sysd in systems:
                if sysd["profile"] is None:
                    F = ref
                else:
                    F = ref + zt[sysd["profile"]] * sysd["wvec"]
                ts = F[ar, tgt]
    ...
    for name, a in acc.items():
        res[name] = {"overall": a["nd"] / n, "hr": a["hit"] / n, "n": n,
                     "tail": a["nd_b"][0] / max(a["n_b"][0], 1),
                     "mid": a["nd_b"][1] / max(a["n_b"][1], 1),
                     "head": a["nd_b"][2] / max(a["n_b"][2], 1),
                     "n_tail": int(a["n_b"][0])}
    return res, per_user
...
def main():
    ...
    print(f"=== COLDFUSE confirm: {category} seed {cfg['seed']} "
          f"(ease={'no' if args.no_ease else 'yes'}) ===", flush=True)
    ...
    bin_idx = np.where(freq <= TAIL_MAX, 0,
                       np.where(freq <= MID_MAX, 1, 2)).astype(np.int64)
    ...
    if args.no_ease:
        we, l2, B_gpu, expected_ref = 0.0, None, None, \
            float(base["best_test"]["NDCG@10"])
        ref_name = "seq"
    else:
        ...
        expected_ref = float(fus["test"]["fused"]["ndcg"])
    ...
    wvecs = []
    for c in candidates():
        wv = np.empty(n_items, dtype=np.float32)
        wv[bin_idx == 0] = c["wt_tail"]
        wv[bin_idx == 1] = c["wt_mid"]
        wv[bin_idx == 2] = c["wt_head"]
        wvecs.append({"name": json.dumps(c, sort_keys=True), "profile":
                      c["profile"], "wvec": torch.from_numpy(wv).to(DEVICE),
                      "combo": c})
    systems_val = [{"name": "REF", "profile": None, "wvec": None}] + wvecs

    print(f"  [val] streaming sweep: {len(systems_val)} systems ...",
          flush=True)
    vres, _ = stream_eval(model, val_users, user_seqs, {}, user_times, {},
                          cfg, pad_id, n_items, E_gpu, B_gpu, we,
                          systems_val, bin_idx,
                          {"dict": val_dict, "bin": bin_idx})
    ref_v = vres["REF"]
    best = None
    for s in wvecs:
        r = vres[s["name"]]
        if (r["overall"] >= ref_v["overall"] - NONINF
                and (best is None or r["tail"] > best[0])):
            best = (r["tail"], s)
    sel = best[1]
    print(f"  VAL ref overall {ref_v['overall']:.5f} tail {ref_v['tail']:.5f}"
          f" | selected {sel['combo']} val tail {best[0]:.5f}", flush=True)

    print("  [test] reference + selected ...", flush=True)
    tres, pu = stream_eval(model, test_users, user_seqs, val_extra,
                           user_times, val_extra_times, cfg, pad_id, n_items,
                           E_gpu, B_gpu, we,
                           [{"name": "REF", "profile": None, "wvec": None},
                            {"name": "SEL", "profile": sel["profile"],
                             "wvec": sel["wvec"]}], bin_idx,
                           {"dict": test_dict, "bin": bin_idx},
                           collect_per_user=True)
    print(f"  TEST ref {tres['REF']['overall']:.5f} (tail "
          f"{tres['REF']['tail']:.5f}) -> sel {tres['SEL']['overall']:.5f} "
          f"(tail {tres['SEL']['tail']:.5f})", flush=True)

    out = Path(args.out) if args.out else Path(args.run_json).with_name(
        f"results_{category}_COLDFUSE_confirm_seed{cfg['seed']}.json")
    report = {"category": category, "seed": cfg["seed"],
              "reference": ref_name, "ease": {"l2": l2, "we": we},
              "expected_ref_overall": expected_ref,
              "noninferiority": NONINF,
              "bins": {"tail_max": TAIL_MAX, "mid_max": MID_MAX},
              "val_sweep": {s["name"]: vres[s["name"]] for s in systems_val},
              "selected": sel["combo"],
              "test": {"reference": tres["REF"], "selected": tres["SEL"]}}
    json.dump(report, open(out, "w", encoding="utf-8"), indent=1)
    tb = np.array([bin_idx[test_dict[u]] for u in test_users], dtype=np.int64)
    np.savez_compressed(str(out)[:-5] + ".perusers.npz",
                        users=np.array(test_users, dtype=np.int64),
                        target_bin=tb, ref_ndcg=pu["REF"], sel_ndcg=pu["SEL"])
