...
- 2026-07-25 ~09:xx (tick) — **repo hygiene: removed the dead BEST-Rec-era figure subsystem** (deposit-readiness prep). The current paper uses only `fig_tail_law_mechanism` (Fig. 1) and `fig_r1r2_plane` (Fig. 2), each with its dedicated generator; but `figures/` still tracked **28 orphaned files** (`fig1_warm_methods`…`fig9_lc2c`, `fig_architecture_overview`) from the superseded multi-figure structure, plus three dead generators (`make_figures.py/v2/v3`). Verified 0 references across md/tex/manifest/deposit-bundle/zenodo/README, and that no required gate (strict/render/build) touches `figures/` or those generators. Removed all 31 + minimally de-wired `run_all.py` (emptied `FIGURE_NAMES`; dropped the `make_figures.py` stage — both remaining consumers guard with `.exists()`, `run_all.py` re-parses clean). `validate_artifacts.py`'s `make_figures_v3` regex is a *forbid* pattern (docs must not reference the old command) so removal is consistent, not a break. strict rebuild exit 0; manifest diff is only the git_commit re-stamp (no figure was ever a manifest entry). No paper/claim touched. Considered but deferred (gated, not tick-sized): M4 v1.1.12 deposit (its own instructions say cut *after* author metadata; DOI needs maintainer login), E-B (VOID + cloud + needs maintainer pod provisioning), E-E V3 (huge, descriptive-only). The stale-claim sweep this tick found the manuscript's pending/queued claims all genuinely open — no new stale-done bug.
- 2026-07-25 (FIR-canonicalization plan, maintainer-authorized; experiments scheduled) — **Phase-2 experiment LAUNCHED.** Froze PREREG_FIR_CANONICAL_BREADTH + committed adjudicator/driver BEFORE launch (a6775c6a); smoke-validated the pipeline on Industrial_and_Scientific (fir-v3 learns, ||Delta||2=1.26, adjudicator schema OK); launched the 32-run canonical-FIR breadth transfer (IS+CDs, identity vs learned canonical --fir-v3, matched-init, 8 fresh seeds, K=16, zero per-category tuning) as a sequential background queue (~6-7h, resume-safe). This is the acceptance-mover: it upgrades the cross-category FIR evidence from the legacy zero-gated package to the canonical gradient-active form. Writing (Phase-1 canonicalization) proceeds in parallel; canonical-breadth RESULTS integrate once the queue completes + adjudicates (likely after 08:00 — training-bound, not a limitation I can compress). Safety archive tag: pre-fir-restructure-20260725.
