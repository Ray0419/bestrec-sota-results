...
`PAPER_SUBMISSION.pdf`; the ACM/TORS source under `paper_tex/` is the venue mirror. Its
quantitative tables are generated, but its prose is a separately maintained TeX mirror
under strict source/PDF semantic checks; the repository does not claim whole-prose
generation. `PAPER_DRAFT.md` is a
...
- Prereg chain: `SOTA_CONFIRM_PREREG_V2.md` (+ `_ERRATA`), `SOTA_CONFIRM_PREREG_OFFICE.md`,
  ...
  `PREREG_WEAREC_BASELINE_V1.md`, `PREREG_EE_V3.md`, and `PREREG_EE_V4.md` (outcome-known
  current-comparator verdicts `_bestrec_run/wearec_baseline_v1_adjudication.json`
  and `_bestrec_run/ee_v{3,4}_adjudication.json`)
- Results of record: `_bestrec_run/results_SOTACONF_V2_*.json` (+ tracked sidecars +
  ...
  the ML-1M README; only aggregate adjudication/provenance is public. E-E V3/V4
  likewise release only compact aggregate adjudications; their TEST endpoints,
  rank sidecars, and checkpoints remain private and are represented by recorded hash ledgers.
...
- The same strict chain now also requires the E-E V4 aggregate verdict
  `EEV4-ALPHAFUSE-ABOVE-NORMAL-SASREC`; its graph cell verifies the V3 adjudication hash,
  recomputes the released normal-init summary and three cross-campaign Welch contrasts,
  and validates the private-ledger schema without reading private endpoints.
...
8. **Current-comparator campaigns (outcome-known same-investigator evidence):** official-code WEARec returned `WEAREC-BELOW-EXISTING-REFERENCE`, 0.059184 [0.058674,0.059693] versus the existing 0.067337 reference. Separately frozen E-E V3 returned `EEV3-REPORTABLE-OUTCOME-KNOWN`: the AlphaFuse-style MiniLM package scored 0.048273 [0.048129,0.048416] versus a zero-initialized upstream-class SASRec-ID control at 0.039024 [0.038106,0.039941], delta +0.009249 [0.008329,0.010169], but remained −0.019065 [−0.019347,−0.018783] below the existing reference. E-E V4 then returned `EEV4-ALPHAFUSE-ABOVE-NORMAL-SASREC`: parser-default `Normal(0,1)` SASRec-ID scored 0.043065 [0.042645,0.043486], leaving the package above it by +0.005207 [+0.004779,+0.005635]; normal-init minus the earlier zero-init arm was +0.004042 [+0.003089,+0.004995]. The V3 result is a whole representation-package contrast; V4 is only a cross-campaign initialization sensitivity. MiniLM replaces the published AlphaFuse text vectors; architecture, text availability, capacity, allocation, and tuning history are not equalized. Official recipes may override initialization by dataset, so the parser setting is not a universal upstream default. Phase/date and initialization are confounded, and an equal-budget factorial remains open. Neither campaign isolates null-space fusion or initialization, supplies independent confirmation, or supports SOTA. Public artifacts are frozen protocols/code and compact adjudications; the graph checks aggregate arithmetic and private-ledger structure while local adjudicators checked the actual private files.
