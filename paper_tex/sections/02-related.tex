...
\item
  The two added components --- the \textbf{strictly causal FIR temporal filter} (confirmed on four categories, two via the pre-declared breadth campaign of \S\ref{sec:5.2}) and \textbf{TAPE} --- plus the \textbf{dataset-conditional long-tail pattern} (\S\ref{sec:5.3}) and its \textbf{interaction-thinning titration} double result (\S\ref{sec:5.4}). The novelty boundary for each is stated explicitly in Table 0 (\S\ref{sec:2.3}).
\end{itemize}
...
\textbf{Novelty-boundary additions (2026-07-19 literature check; identifiers cited pending full bibliography entries in the citations round).} Five adjacent lines narrow what this paper may claim, and we state the narrowed boundary explicitly. (i) \emph{Text-similarity for rare/unseen items:} SimRec (arXiv:2410.22136) already targets rare and unseen sequential-recommendation items with text-derived item similarity and reports larger gains on sparser data; our contribution is therefore \textbf{not} the generic sparse-vs-dense semantic-tail idea but the specific AR2023 full-catalog HSTU-style cross-category contrast with its intervention studies and disclosed cohort defects (\S\ref{sec:5.3}--\S\ref{sec:5.4}). (ii) \emph{Learnable frequency filtering:} TASIF (arXiv:2512.24246), DLFS-Rec (RecSys 2023, DOI 10.1145/3604915.3608782), and DIFF (arXiv:2505.13974) use learnable frequency-domain filters, gates, and filter-plus-side-information fusion; our defensible distinction is only the \textbf{left-causal depthwise FIR realization under an HSTU-style all-position objective} --- not learnable filtering, gating, or filter-content fusion as concepts. (iii) \emph{Side-information fusion scope:} NOVA (AAAI 2021) and DIF-SR (SIGIR 2022) document that direct/early fusion can be neutral or harmful and propose decoupled alternatives; we test \textbf{only early additive frozen-text fusion}, so every ``text helps/does not help'' statement in this paper is scoped to that construction. (iv) \emph{Direct sequential cold-start methods} (e.g., Mecos, AAAI 2021; RecGPT, EMNLP 2025) address true zero-history items; our tail cohort mixes zero-train-exposure and low-frequency items (\S\ref{sec:5.3}), and we make no cold-start-method comparison. (v) \emph{Reproducibility apparatus:} accountable-workflow and reproduction-audit precedents exist (Bellog\'in \& Said 2021; Petrov \& Macdonald, RecSys 2022); what this paper claims is only its exact combined enforcement mechanism --- per-cell recomputation from released artifacts, provenance tripwires, and symmetric self-VOIDing adjudication --- not the general concept of reproducible or accountable evaluation. Relatedly, the chunked-full-softmax loss is implementation engineering with published memory-efficient exact-CE precedent (Cut Cross-Entropy, ICLR 2025), and we cite it as such (\S\ref{sec:3.5}).
