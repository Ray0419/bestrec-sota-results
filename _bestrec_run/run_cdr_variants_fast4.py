"""Optimized CDR variant driver: fast3 + user subsampling in inner validation.

Inner-validation alpha selection only needs to discriminate among 9 alphas;
NDCG@10 estimates with n=2000 users are sufficient (the median-rank-based
Wilcoxon decision used at outer time is a different concern).

This driver subsamples users in inner-validation to MAX_INNER_VAL_USERS
(default 2500) using a deterministic per-fold RNG seed. Outer evaluation is
NOT subsampled — full 14K users (or whatever).

Validation: same alpha grid; same Holm-corrected Wilcoxon comparison at outer.
"""
...
def eval_alphas_at_once(variant_name, Xw, B_warm, warm_indices, cold_indices,
                          ...
                          fold_id=None, dataset=None, max_users=None,
                          subsample_rng=None):
    ...
    if max_users is not None and subsample_rng is not None and len(users) > max_users:
        idx = subsample_rng.choice(len(users), max_users, replace=False)
        users = sorted([users[i] for i in idx])
    ...
    for s in range(0, len(users), batch_size):
        ...
        cd_cold_raw = Xw[uids] @ S_w_c
        lc_cold_raw = Xw[uids] @ B_hat_cold
        cd_z = _z(cd_cold_raw)
        lc_z = _z(lc_cold_raw)
...
def run(dataset, seeds, variants, perpair_out=None,
        max_inner_val_users=MAX_INNER_VAL_USERS):
    print(f"\n{'#'*70}\n# CDR variants (fast4 / inner subsample {max_inner_val_users}): {dataset.upper()}\n{'#'*70}",
            flush=True)
        ...
        for fold_id, (tr, te, cold) in enumerate(splits):
            ...
            for variant_name in variants:
                ...
                if cache_in is None:
                    ...
                    sub_rng = np.random.RandomState((seed * 1000 + fold_id + 1234) & 0xFFFFFFFF)
                    val_res, _, _ = eval_alphas_at_once(
                        ...
                        max_users=max_inner_val_users, subsample_rng=sub_rng)
                    val_scores = {a_: val_res[a_]["NDCG@10"] for a_ in CDR_ALPHA_GRID}
                ...
                outer_res, outer_pu, outer_rows = eval_alphas_at_once(
                    ...
                    dataset=dataset)
                summ = outer_res[a]; pu = outer_pu[a]
                ...
                print(f"  seed={seed} fold={fold_id} variant={variant_name:>14s} "
                      ...
                      f"HR@10={summ['HR@10']:.4f} TOTAL={t_var:.1f}s val_n_eval={val_res[CDR_ALPHA_GRID[0]]['n_eval'] if cache_in else 0}",
                      flush=True)
    ...
    return {"perfold": {n: {k: list(map(float, v)) for k, v in d.items()}
            ...
            "inner_val_subsample": max_inner_val_users}
...
def main():
    ...
    ap.add_argument("--max-inner-val-users", type=int, default=MAX_INNER_VAL_USERS)
    args = ap.parse_args()
    ...
    for ds in args.datasets:
        ...
        payload["datasets"][ds] = run(ds, seeds, variants, perpair_out=perpair_out,
                                        max_inner_val_users=args.max_inner_val_users)
