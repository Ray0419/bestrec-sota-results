...
# K-DIAL COPY NOTE (2026-08-05): a PARALLEL SESSION placed its own
# PAUSE_EXPERIMENTS sentinel in this worktree root to serialize GPU access. That
# file is deliberately left untouched — it is not this session's to remove, and
# the main-checkout trainer still honors it. This copy therefore watches its own
# sentinel name, and the *intent* of the shared lock (no GPU contention) is
# enforced more precisely by poc_kdial5_driver.py, which probes nvidia-smi and
# refuses to launch whenever the GPU is busy, before every single run.
...
ROOT = Path(r"C:\Users\rayxc\Documents\R")  # K-DIAL copy: data lives in the main checkout
...
def main():
    ...
    ap.add_argument("--item-cap-k", type=int, default=0,
                     help="K-DIAL: cap TRAIN occurrences of selected items to the "
                          "most recent K (0 = remove from training entirely). "
                          "Train-only thinning: eval histories + terciles frozen "
                          "at full density, same contract as --subsample-train-frac.")
    ap.add_argument("--item-cap-frac", type=float, default=0.0,
                     help="K-DIAL: fraction of eligible items (full train degree >= "
                          "--item-cap-min-deg) to cap. 0 disables (bit-identical no-op).")
    ap.add_argument("--item-cap-seed", type=int, default=7,
                     help="K-DIAL: RNG seed for the capped-item selection.")
    ap.add_argument("--item-cap-min-deg", type=int, default=8,
                     help="K-DIAL: eligibility floor on full train degree (so the cap binds).")
    ap.add_argument("--item-cap-multi", type=str, default="",
                     help="PREREG_DOSE_RESPONSE_V1: comma-separated dose levels, e.g. "
                          "'0,1,2,4,8,16'. Partitions a sample of eligible items into "
                          "DISJOINT equal groups, one per dose, plus a matched uncapped "
                          "control group -- so a single run yields the whole dose-response "
                          "at constant global density. --item-cap-frac is the per-group "
                          "fraction of eligible items. Overrides --item-cap-k.")
    ...
    # K-DIAL (POC 2026-08-05): graded item-degree knockdown. For a deterministic
    # sample of eligible items, keep only the most recent --item-cap-k TRAIN
    # occurrences (k=0 removes the item from training entirely). Same train-only
    # contract as the titration above: user_seqs / eval histories / terciles stay
    # full-density; only train_user_seqs is rebuilt. Not composable with
    # --subsample-train-frac < 1 in this POC.
    if args.item_cap_frac > 0.0:
        assert args.subsample_train_frac >= 1.0, "k-dial POC: no composition with subsample"
        _full_deg = {}
        for (_, _it, _, _) in train_inters:
            _full_deg[_it] = _full_deg.get(_it, 0) + 1
        _elig = np.array(sorted(i for i, d in _full_deg.items()
                                if d >= args.item_cap_min_deg), dtype=np.int64)
        _rng2 = np.random.RandomState(args.item_cap_seed)
        _n_cap = int(round(args.item_cap_frac * len(_elig)))
        # item -> its dose k. Single-dose mode keeps the original behaviour.
        _dose = {}
        _groups = {}
        if args.item_cap_multi:
            _doses = [int(x) for x in args.item_cap_multi.split(",")]
            _need = _n_cap * (len(_doses) + 1)      # +1 = matched uncapped control
            assert _need <= len(_elig), (
                f"multi-dose needs {_need} eligible items (per-group {_n_cap} x "
                f"{len(_doses)} doses + control) but only {len(_elig)} have degree "
                f">= {args.item_cap_min_deg}; lower --item-cap-frac")
            _pick = _rng2.choice(_elig, size=_need, replace=False)
            for _gi, _k in enumerate(_doses):
                _g = _pick[_gi * _n_cap:(_gi + 1) * _n_cap]
                _groups[str(_k)] = sorted(int(x) for x in _g)
                for _it in _g:
                    _dose[int(_it)] = _k
            _groups["control"] = sorted(
                int(x) for x in _pick[len(_doses) * _n_cap:])
        else:
            _g = _rng2.choice(_elig, size=_n_cap, replace=False)
            _groups[str(args.item_cap_k)] = sorted(int(x) for x in _g)
            for _it in _g:
                _dose[int(_it)] = args.item_cap_k
        _cap_set = set(_dose)
        _seen_cap = {}
        _keep = [True] * len(train_inters)
        _by_time = sorted(range(len(train_inters)),
                          key=lambda j: -train_inters[j][3])
        for _ix in _by_time:                       # newest first
            _it = train_inters[_ix][1]
            if _it in _cap_set:
                _c = _seen_cap.get(_it, 0)
                if _c >= _dose[_it]:
                    _keep[_ix] = False
                else:
                    _seen_cap[_it] = _c + 1
        train_inters_cap = [r for r, kf in zip(train_inters, _keep) if kf]
        train_user_seqs = build_user_sequences(train_inters_cap)
        train_user_times = (build_user_time_sequences(train_inters_cap)
                            if (args.time_bias or args.time_decay_kernel) else None)
        _capset_path = str(args.out) + ".capset.json"
        with open(_capset_path, "w", encoding="utf-8") as _f:
            json.dump({"item_cap_k": args.item_cap_k,
                       "item_cap_multi": args.item_cap_multi,
                       "item_cap_frac": args.item_cap_frac,
                       "item_cap_seed": args.item_cap_seed,
                       "item_cap_min_deg": args.item_cap_min_deg,
                       "n_eligible": int(len(_elig)),
                       "per_group_n": int(_n_cap),
                       "groups": _groups,
                       "capped_items": sorted(int(i) for i in _cap_set)}, _f)
        if args.item_cap_multi:
            print(f"  K-DIAL multi-dose [{args.item_cap_multi}]: {_n_cap:,} items per "
                  f"group x {len(_groups)-1} doses + {len(_groups['control']):,} "
                  f"uncapped control, from {len(_elig):,} eligible (min-deg "
                  f"{args.item_cap_min_deg}, seed {args.item_cap_seed}) -> "
                  f"{len(train_inters_cap):,}/{len(train_inters):,} train "
                  f"interactions kept ({100*len(train_inters_cap)/len(train_inters):.1f}%); "
                  f"capset -> {_capset_path}; eval FROZEN at full density")
        else:
            print(f"  K-DIAL: capped {len(_cap_set):,}/{len(_elig):,} eligible items to "
                  f"k<={args.item_cap_k} (min-deg {args.item_cap_min_deg}, seed "
                  f"{args.item_cap_seed}) -> {len(train_inters_cap):,}/{len(train_inters):,} "
                  f"train interactions kept; capset -> {_capset_path}; "
                  f"eval histories + terciles FROZEN at full density")
