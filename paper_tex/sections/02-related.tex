...
\item
  The two added components --- the \textbf{strictly causal FIR temporal filter} (positive matched identity contrasts on three outcome-visible categories; active controls do not isolate temporal specificity or learned taps; \S\ref{sec:5.2}) and \textbf{TAPE} --- plus the \textbf{MI frequency-5 tail case} (\S\ref{sec:5.3}) and its \textbf{interaction-thinning titration} level-contrast result (\S\ref{sec:5.4}). The novelty boundary for each is stated explicitly in Table 0 (\S\ref{sec:2.3}).
\end{itemize}
...
available controls do not isolate temporal specificity from trainable-residual capacity
