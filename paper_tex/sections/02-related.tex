...
\item
  The two added components --- the \textbf{strictly causal FIR temporal filter} (positive matched identity contrasts on three outcome-visible categories; an equal-parameter non-temporal placebo supports temporal access in MI, while the shared causal control prevents per-channel-tap attribution; \S\ref{sec:5.2}) and \textbf{TAPE} --- plus the \textbf{MI frequency-5 tail case} (\S\ref{sec:5.3}) and its \textbf{interaction-thinning titration} level-contrast result (\S\ref{sec:5.4}). The novelty boundary for each is stated explicitly in Table 0 (\S\ref{sec:2.3}).
\end{itemize}
...
an equal-parameter current-position-only placebo supports temporal access relative to
that placebo, while a shared causal filter prevents attribution to per-channel taps
