...
All datasets are AR2023 5-core categories under the same full-catalog LLOO protocol (\S\ref{sec:3.6}). Counts are \textbf{total 5-core interactions} (train = total \ensuremath{-} 2\ensuremath{\times}users, since LLOO holds out one valid and one test interaction per user):

\input{tables/table_datasets41}

Video\_Games/Musical\_Instruments/Office\_Products match the HSTU-BLaIR comparator pipeline's own statistics exactly on users/items (interactions within \ensuremath{\pm}1; \S\ref{sec:5.2}). The Beauty\_and\_\allowbreak PC catalog is 8\ensuremath{\times} larger than Video\_Games, with median user history of only 5 interactions --- a harder benchmark. Every pre-registered campaign in this version is fully adjudicated (verdicts in \S\ref{sec:5.2} and Appendix A.0).
