...
\textbf{Clean-ablation finding (variant 21, added after audit)}: When we hold the encoder constant at MiniLM and ONLY change projection layer from single-Linear to the SASRecText 2-layer MLP adaptor, NDCG@10 drops slightly from \textasciitilde0.0190 to 0.01889. \textbf{No improvement from the MLP adaptor was observed in this single comparison.} The +1.3--2.3\% gains in variants 16/19/20 therefore cannot be attributed to the projection layer alone; their bundled encoder/content changes and possible interactions remain unresolved.
...
\tightlist
\item
  \textbf{MLP adaptor alone}: no improvement observed in the single comparison (variant 21 vs 7 = -1.2\%)
\item
...
\subsection*{A.2 Beauty\_and\_\allowbreak PC multi-seed signal (superseded supporting material)}
