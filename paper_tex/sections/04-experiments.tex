...
Video\_Games/Musical\_Instruments/Office\_Products match the HSTU-BLaIR comparator pipeline's own statistics exactly on users/items (interactions within \ensuremath{\pm}1; \S\ref{sec:5.2}). The Beauty\_and\_\allowbreak PC catalog is 8\ensuremath{\times} larger than Video\_Games, with median user history of only 5 interactions --- a harder benchmark. Every pre-declared campaign that has completed is mechanically adjudicated (verdicts in \S\ref{sec:5.2} and Appendix A.0).
