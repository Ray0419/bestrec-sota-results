...
| 3 | **Claim overreach CLOSED; experiment OPEN** | The paper now says only that the tested trainable left-causal residual arms improved frozen identity in this setup. It states that shared/nonlinear arms were not separated from learned taps, MA/HP are redundant, and the absent active lag-0/pointwise non-temporal placebo prevents temporal-specific attribution. A fail-closed no-future-leakage unit test now perturbs future tokens with every filter/control path active and verifies invariant prefix states under both Transformer and HSTU encoders (16/16 paths PASS); this establishes implementation causality, not temporal-mechanism specificity. Selected-checkpoint tap/frequency-response summaries are released as explicitly descriptive diagnostics with intervals over eight seed summaries. A new matched placebo study remains required. |
