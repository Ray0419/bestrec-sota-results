"""Compute Holm-corrected per-USER paired Wilcoxon for LightGCN-strict vs
...
Inputs:
  ...
  results_lightgcn_strict_perfold_<ds>.json - lightgcn_strict per-(user, seed)

Output:
  results_lightgcn_strict_wilcoxon.json    - per-dataset paired wilcoxon stats

Per-user aggregation:
  - ease_sbert: mean across folds for each user_id  (n_users measurements)
  - lightgcn_strict: mean across (seed, fold) for each user_id  (n_users measurements)
...
def load_lightgcn_per_user(perfold_path):
    """Return user_id -> mean NDCG@10 over (seed, fold) for lightgcn_strict.
...
def holm(pvals):
    """Holm-Bonferroni step-down adjustment. Returns adjusted p-values, same length."""
    m = len(pvals)
...
def main():
    ...
    for ds in args.datasets:
        ...
        gcn_path  = os.path.join(root, f'results_lightgcn_strict_perfold_{ds}.json')
        if not os.path.exists(ease_path):
            print(f'  SKIP {ds}: missing {ease_path}')
            continue
        if not os.path.exists(gcn_path):
            print(f'  SKIP {ds}: missing {gcn_path}')
            continue
        ease = load_ease_per_user(ease_path, method='ease_sbert')
        gcn  = load_lightgcn_per_user(gcn_path)
        common = sorted(set(ease.keys()) & set(gcn.keys()))
        ease_v = np.array([ease[u] for u in common])
        gcn_v  = np.array([gcn[u]  for u in common])
        diff   = gcn_v - ease_v   # positive = lightgcn wins
        n_users = len(common)
        n_pos = int((diff > 0).sum())
        n_neg = int((diff < 0).sum())
        n_zero = int((diff == 0).sum())
        gcn_mean  = float(np.mean(gcn_v))
        ...
        raw[ds] = {
            ...
            'lightgcn_strict_NDCG@10_per_user_mean': gcn_mean,
            'mean_diff_lightgcn_minus_ease_sbert': float(np.mean(diff)),
            'n_lightgcn_wins': n_pos,
            'n_ease_sbert_wins': n_neg,
            'n_ties': n_zero,
            'wilcoxon_two_sided_p': p_two,
            'wilcoxon_lightgcn_greater_p': p_gt,
            'wilcoxon_lightgcn_less_p': p_lt,
            'wilcoxon_statistic': w_stat,
        }
        print(f'  {ds:>13s}: ease_sbert={ease_mean:.4f}  lightgcn={gcn_mean:.4f}  '
              f'diff={float(np.mean(diff)):+.4f}  wins L/E={n_pos}/{n_neg}  '
    ...
    if keys:
        ...
        pvals_gt = np.array([raw[k]['wilcoxon_lightgcn_greater_p'] for k in keys])
        adj_gt = holm(pvals_gt)
        for k, a in zip(keys, adj_gt):
            raw[k]['wilcoxon_lightgcn_greater_p_holm'] = float(a)
        pvals_lt = np.array([raw[k]['wilcoxon_lightgcn_less_p'] for k in keys])
        adj_lt = holm(pvals_lt)
        for k, a in zip(keys, adj_lt):
            raw[k]['wilcoxon_lightgcn_less_p_holm'] = float(a)

    out_path = os.path.join(root, 'results_lightgcn_strict_wilcoxon.json')
    with open(out_path, 'w') as f:
        json.dump({
            'note': ('Per-USER paired Wilcoxon for tuned LightGCN vs EASE+SBERT '
                     'on warm-LOO. Per-user NDCG@10 is the mean over folds '
                     '(ease_sbert) or over (seed, fold) (lightgcn_strict). '
                     'Holm-Bonferroni correction is applied ACROSS the per-dataset '
                     'p-values for each of the three alternatives.'),
            'methods_compared': ['lightgcn_strict (multi-seed multi-config tuned)',
