...
This protocol shares chronological leave-last-out structure with TIGER \citep{rajput2023tiger} and LIGER \citep{yang2024liger}, but it is not identical to either: our iterative filtering applies to users and items, matching LIGER's stated k-core dimension. The TIGER paper states user filtering, but the exact geometry of its released preprocessing is not established here; both comparator papers use Amazon Reviews 2014 rather than AR2023. \citet{hou2024blair}'s AR2023 reference repo (\texttt{external/\allowbreak AmazonReviews2023/\allowbreak seq\_rec\_results/}) uses the \texttt{0core\_timestamp\_w\_his\_*} HuggingFace subset. These pipelines produce different catalogs and cannot support direct numerical comparisons.
...
\end{equation*}
Identity initialization is a controlled no-op choice, not an optimality claim. \citet{tian2026convllm} concurrently report that random initialization outperforms bias-free zero-weight initialization for their selected residual depthwise Conv1D in Qwen3. Their selected module is also of the form $x+\operatorname{Conv}(x)$, making the zero-weight case structurally relevant, but the task, domain, post-QKV placement, model scale, kernel, optimizer, and presence/absence of a trainable bias differ. We did not run a random-initialized canonical FIR arm and therefore do not claim that $\Delta=0$ is generally preferable.
...
A separate, subsequently frozen Software attempt also evaluated the canonical K=16 form and returned a positive thresholded verdict. Because earlier V2 validation-output non-visibility is not independently established, it is classified outcome-known/exploratory same-team robustness; its same-investigator, same-code-lineage, same-Amazon-family, local-custody scope also precludes independent-confirmation wording (\S\ref{sec:5.2}).

Both components are leak-free by construction and add negligible parameters (TAPE: K\ensuremath{\times}d \ensuremath{\approx} 33k; filter: K\ensuremath{\times}d \ensuremath{\approx} 0.5--3k). Two further regularizers we use are \emph{not} claimed novel: \textbf{label smoothing} \citep{szegedy2016rethinking} on the full-softmax CE target, and the standard time / text-similarity / relative-position attention biases (TiSASRec \citep{li2020tisasrec}; HSTU \citep{zhai2024hstu}; \citealp{shaw2018relative}), each cited at the point of use.
