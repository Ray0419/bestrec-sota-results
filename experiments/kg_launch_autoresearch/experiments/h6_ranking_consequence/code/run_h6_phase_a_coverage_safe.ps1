...
$selfTestRoot = Join-Path $h6Root 'selftest_artifacts'
$artifactRoot = if ($SelfTest) { $selfTestRoot } else { $resultsRoot }
[System.IO.Directory]::CreateDirectory($artifactRoot) | Out-Null
$runner = (Resolve-Path -LiteralPath (Join-Path $here 'run_h6_phase_a_coverage.py')).Path
...
$correction = (Resolve-Path -LiteralPath (Join-Path $h6Root 'correction_run_002_runtime.md')).Path
...
$outerLock = if ($SelfTest) {
    Join-Path $h6Root 'H6_PYTHON_SELFTEST_LAUNCH.lock'
}
else {
    Join-Path $h6Root 'H6_COVERAGE_RUN_002_LAUNCH.lock'
}
$innerLock = Join-Path $h6Root 'H6_COVERAGE_RUN_002_INNER.lock'
$primaryDirectory = Join-Path $resultsRoot 'coverage_run_002'
$replayDirectory = Join-Path $resultsRoot 'coverage_run_002_replay'
$confirmatoryCompletionMarker = Join-Path $resultsRoot 'coverage_run_002_completion.json'
...
function Assert-FixedHashes {
    ...
    if ((Get-Sha256 -Path $correction) -ne $Runtime.correction_sha256) { throw 'Runtime correction changed during launch.' }
...
function Assert-StartRecord {
    ...
    if (
        $Start.mode -ne 'h6-child-process-start' -or
    ...
    foreach ($name in @('runner', 'protocol', 'correction', 'launcher', 'base_interpreter', 'venv_launcher', 'venv_config')) {
        $expectedHashName = $name + '_sha256'
...
function Start-H6Child {
    ...
    $authorization = [ordered]@{
        mode = 'h6-launcher-authorization'
        action = $Action
        launch_id = $LaunchId
    ...
    try {
        ...
        Write-ExclusiveJson -Path $ackPath -Value ([ordered]@{
            mode = 'h6-child-process-acknowledgement'
            action = $Action
            launch_id = $LaunchId
            launcher_pid = [int]$PID
...
$runtime = [ordered]@{
    ...
    correction_sha256 = Get-Sha256 -Path $correction
...
$launchPrefix = if ($SelfTest) { 'st_' } else { 'r2_' }
...
$completionMarker = if ($SelfTest) {
    Join-Path $launchDirectory 'selftest_completion.json'
}
else {
    $confirmatoryCompletionMarker
}
$outerLockMode = if ($SelfTest) {
    'h6-python-selftest-outer-lock'
}
else {
    'h6-coverage-run-002-outer-lock'
}
...
try {
    ...
    if ($SelfTest) {
        ...
        $selfTestChild = Start-H6Child `
            ...
            -ExtraAuthorization ([ordered]@{ self_test_only = $true })
        if ($selfTestChild.summary.status -ne 'H6_PHASE_A_PYTHON_SELFTEST_COMPLETE') {
            throw 'Isolated Python self-test summary identity mismatch.'
        }
        Assert-FixedHashes -Runtime $runtime
        $completionPayload = [ordered]@{
            schema_version = 'h6_python_selftest_completion.v1'
            status = 'H6_PYTHON_SELFTEST_COMPLETE'
            launch_id = $launchId
            launcher_pid = [int]$PID
            completed_unix_ns = Get-UnixTimeNs
            isolation = [ordered]@{
                confirmatory_result_directories_opened = $false
            ...
            runtime = [ordered]@{
                ...
                runtime_correction = Get-FileRecord -Path $correction
    ...
    $primary = Start-H6Child `
        ...
        -LaunchId $launchId `
        -LaunchDirectory $launchDirectory `
        -ExtraAuthorization ([ordered]@{ output_id = 'coverage_run_002'; run_role = 'primary' })
    if ($primary.summary.status -ne 'H6_PHASE_A_RUN_COMPLETE' -or
        $primary.summary.output_id -ne 'coverage_run_002' -or
    ...
    $replay = Start-H6Child `
        ...
        -LaunchId $launchId `
        -LaunchDirectory $launchDirectory `
        -ExtraAuthorization ([ordered]@{ output_id = 'coverage_run_002_replay'; run_role = 'exact_replay' })
    if ($replay.summary.status -ne 'H6_PHASE_A_RUN_COMPLETE' -or
        $replay.summary.output_id -ne 'coverage_run_002_replay' -or
    ...
    if (
        [string]$primary.summary.manifest_sha256 -ne [string]$replay.summary.manifest_sha256 -or
        [string]$primary.summary.metric_payload_sha256 -ne [string]$replay.summary.metric_payload_sha256
    ) {
    ...
    if ($verifier.summary.status -ne 'H6_PHASE_A_VERIFY_COMPLETE') {
        throw 'Deep-verifier summary identity mismatch.'
    }
    if (
        [string]$verifier.summary.manifest_sha256 -ne [string]$primary.summary.manifest_sha256 -or
        [string]$verifier.summary.metric_payload_sha256 -ne [string]$primary.summary.metric_payload_sha256
    ) {
    ...
    foreach ($directory in @($primaryDirectory, $replayDirectory)) {
        ...
        if (($artifactNames -join '|') -ne 'cohort_manifest.jsonl|result.json') {
    ...
    $completionPayload = [ordered]@{
        schema_version = 'h6_coverage_run_002_completion.v1'
        status = 'H6_COVERAGE_RUN_002_COMPLETE'
        launch_id = $launchId
        launcher_pid = [int]$PID
        completed_unix_ns = Get-UnixTimeNs
        execution_contract = [ordered]@{
            engine = 'single-process NumPy-vectorized exact 50-anchor Gram kernel'
            parity_claim = 'semantic and algebraic parity established by exhaustive synthetic self-tests; exact same-engine replay required'
            python_flags = @('-B', '-u')
        ...
        outputs = [ordered]@{
            ...
            primary_manifest = Get-FileRecord -Path (Join-Path $primaryDirectory 'cohort_manifest.jsonl')
            replay_result = Get-FileRecord -Path (Join-Path $replayDirectory 'result.json')
            replay_manifest = Get-FileRecord -Path (Join-Path $replayDirectory 'cohort_manifest.jsonl')
            deep_verification = Get-FileRecord -Path $verificationPath
        }
        exact_replay = [ordered]@{
            manifest_sha256 = [string]$verifier.summary.manifest_sha256
            metric_payload_sha256 = [string]$verifier.summary.metric_payload_sha256
            verified = $true
        }
        decision = [ordered]@{
            verdict = [string]$primaryResult.decision.verdict
            labels_may_be_opened = [bool]$primaryResult.decision.labels_may_be_opened
            replay_verdict = [string]$replayResult.decision.verdict
        }
        completion_marker_is_last = $true
        outer_lock_release = $null
    }
