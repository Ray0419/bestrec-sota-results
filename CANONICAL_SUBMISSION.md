...
5. Reproducibility: provenance manifests, per-user sidecars (MI tracked; Office/FIR-breadth
   ...
   tracked/released-artifact boundary is the reproducibility contract: the 192 declared
...
- Results of record: `_bestrec_run/results_SOTACONF_V2_*.json` (+ tracked sidecars +
  ...
  `_bestrec_run/hstu_results_manifest.json` (17 required claim families)
- Table generation (fail-closed): `_bestrec_run/build_hstu_tables.py` regenerates every
  ...
  (invariants: 0 mismatch / 0 untraceable / all 18 required families sourced; the authoritative cell count is the strict build's own output — 192 at this writing, and it grows as evidence lands)
