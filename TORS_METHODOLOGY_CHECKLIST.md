...
| Exact-input temporal isolation | OPEN | Prior pointwise control changes more than temporal access. A closer repeated-current draft exists. | Claude must approve or reject the estimand; human must authorize data/custody; Codex may freeze and run only after all prelaunch gates pass. |
