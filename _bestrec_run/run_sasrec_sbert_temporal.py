...
# TEMPORAL COPY NOTE: the parallel session's PAUSE_EXPERIMENTS sentinel in this
# worktree root is deliberately left untouched (not ours to remove). This copy
# watches its own sentinel; GPU serialization is enforced by the driver's
# nvidia-smi probe before every run.
if __name__ == "__main__":
    _pause = os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir,
                           "PAUSE_POC_EXPERIMENTS")
    if os.path.exists(_pause):
        sys.stderr.write("PAUSE_POC_EXPERIMENTS sentinel present — POC "
                         "experiments are paused; exiting without running.\n")
        raise SystemExit(0)
# --------------------------------------------------------------------------
...
ROOT = Path(r"C:\Users\rayxc\Documents\R")  # TEMPORAL copy: data lives in the main checkout
...
class SASRecSBERT(nn.Module):
    ...
    def __init__(self, n_items, pad_id, max_seq_len, d_model=64, n_layers=2,
        ...
        self.fir_control_group_kernels = None
        self.fir_control_lowrank_left = None
        self.fir_control_lowrank_right = None
        if self.fir_control != "off":
            if self.fir_control not in {
                    "identity", "learned", "fixed_ma", "fixed_hp",
                    "shared", "grouped", "lowrank", "nonlinear",
                    "pointwise"}:
            ...
            if self.fir_control in {"identity", "learned", "nonlinear"}:
                ...
                with torch.no_grad():
                    self.fir_control_module.weight.zero_()
            elif self.fir_control == "grouped":
                Gc = int(fir_control_groups)
                if Gc < 1 or Gc > d_model or d_model % Gc:
                    raise ValueError(
                        "fir_control_groups must divide d_model and lie in "
                        "[1,d_model]")
                self.fir_control_groups = Gc
                self.fir_control_group_kernels = nn.Parameter(
                    torch.zeros(Gc, Kc))
                self.register_buffer(
                    "fir_control_channel_group",
                    torch.arange(d_model, dtype=torch.long) * Gc // d_model)
            elif self.fir_control == "lowrank":
                Rc = int(fir_control_rank)
                if Rc < 1 or 2 * Rc > min(d_model, Kc):
                    raise ValueError(
                        "fir_control_rank must be >=1 and 2*rank must not "
                        "exceed min(d_model,kernel)")
                self.fir_control_rank = Rc
                # Tangent-factorized channel-by-lag coefficient matrix:
                # W = A V0 + U0 B.  A and B are zero initialized; U0 and V0
                # are fixed orthonormal DCT bases.  The residual is therefore
                # exact identity at step 0, both learned factors have a
                # nonzero gradient path at that point, and rank(W) <= 2R.
                cd = torch.arange(d_model, dtype=torch.float32).view(-1, 1)
                rd = torch.arange(Rc, dtype=torch.float32).view(1, -1)
                u0 = math.sqrt(2.0 / d_model) * torch.cos(
                    math.pi * (cd + 0.5) * rd / d_model)
                u0[:, 0].fill_(math.sqrt(1.0 / d_model))
                ck = torch.arange(Kc, dtype=torch.float32).view(1, -1)
                rk = torch.arange(Rc, dtype=torch.float32).view(-1, 1)
                v0 = math.sqrt(2.0 / Kc) * torch.cos(
                    math.pi * (ck + 0.5) * rk / Kc)
                v0[0].fill_(math.sqrt(1.0 / Kc))
                self.register_buffer("fir_control_lowrank_u0", u0)
                self.register_buffer("fir_control_lowrank_v0", v0)
                self.fir_control_lowrank_left = nn.Parameter(
                    torch.zeros(d_model, Rc))
                self.fir_control_lowrank_right = nn.Parameter(
                    torch.zeros(Rc, Kc))
            else:
    ...
    def item_features(self, item_ids):
        ...
        # PREREG_TRAINED_V1 arm T1 ("Let It Go"-style content anchor, credited):
        # base = text projection, plus a norm-clipped trainable delta. The later
        # additive-sbert block is skipped in this mode (no double-add).
        if getattr(self, "content_anchor", False) and self.use_sbert:
            delta = e
            dn = delta.norm(dim=-1, keepdim=True)
            delta = delta * (self.delta_max / dn.clamp_min(1e-8)).clamp(max=1.0)
            e = self.sbert_proj(self.sbert_table(item_ids)) + delta
        ...
        if getattr(self, "conn_gate", False) and self.use_sbert:
            ...
            e = w_id * e + w_tx * self.sbert_proj(self.sbert_table(item_ids))
        elif self.use_sbert and not getattr(self, "content_anchor", False):
            s = self.sbert_table(item_ids)              # (..., sbert_dim)
            e = e + self.sbert_proj(s)
        if m is not None:
            e = e + m                                   # additive TAPE term, unchanged
        # PREREG_TRAINED_V1 arms T2a/T2b: pseudo-cold replacement (DropoutNet-
        # style simulation, credited; REPLACE not add, per the k=0 finding).
        # Training-time only; the mask is resampled each epoch in main().
        if (self.training and getattr(self, "pseudo_cold_frac", 0.0) > 0.0
                and self.use_sbert):
            pm = self.pseudo_cold_mask[item_ids]
            if bool(pm.any()):
                rep = self.sbert_proj(self.sbert_table(item_ids))
                e = torch.where(pm.unsqueeze(-1), rep, e)
        return e
    ...
    def encode(self, input_ids, times=None):
        ...
        if self.fir_control != "off":
            ...
            if self.fir_control in {"identity", "learned"}:
                ...
                weight = self.fir_control_group_kernels[
                    self.fir_control_channel_group].unsqueeze(1)
                delta = F.conv1d(xp, weight, groups=self.d_model)
                x = x + delta.transpose(1, 2)
            elif self.fir_control == "lowrank":
                weight = (
                    self.fir_control_lowrank_left
                    @ self.fir_control_lowrank_v0
                    + self.fir_control_lowrank_u0
                    @ self.fir_control_lowrank_right).unsqueeze(1)
                delta = F.conv1d(xp, weight, groups=self.d_model)
                x = x + delta.transpose(1, 2)
            else:
...
def train_one_epoch(model, loader, opt, pad_id, device, grad_clip=5.0,
    ...
    for batch in loader:
        ...
        if getattr(model, "gbce_t", -1.0) >= 0.0:
            # PREREG_LOSS_V1: gBCE (gSASRec, Petrov & Macdonald RecSys 2023 /
            # IJCAI 2024, credited). Negative-sampling BCE with the positive
            # term raised to power beta, calibrating away overconfidence:
            #   alpha = k / (n_items - 1);  beta = alpha * (t*(1 - 1/alpha) + 1/alpha)
            #   L = -[ beta*log sigma(s+) + mean_neg log(1 - sigma(s-)) ]
            # t=1 -> beta=1 -> plain BCE; smaller t -> stronger calibration.
            hidden = model.encode(inputs, times=times)
            valid = (targets != pad_id)
            if not valid.any():
                continue
            h_v = hidden[valid]
            pos_ids = targets[valid]
            K = max(int(model.gbce_negs), 1)
            negs = torch.randint(0, model.n_items, (h_v.shape[0], K),
                                 device=device)
            pos_e = model.item_features(pos_ids)                 # (nv, d)
            neg_e = model.item_features(negs)                    # (nv, K, d)
            s_pos = (h_v * pos_e).sum(-1)                        # (nv,)
            s_neg = torch.einsum("nd,nkd->nk", h_v, neg_e)       # (nv, K)
            alpha = K / max(model.n_items - 1, 1)
            t_cal = float(model.gbce_t)
            beta = alpha * (t_cal * (1.0 - 1.0 / alpha) + 1.0 / alpha)
            # numerically stable: log sigma(x) = -softplus(-x)
            loss = (beta * F.softplus(-s_pos)).mean() \
                + F.softplus(s_neg).mean()
        elif chunked_full_softmax:
            ...
            # PREREG_TRAINED_V1 arm T2b: within-pool CE — the offset-invariant
            # evaluation metric expressed as a training objective. For targets
            # that are pseudo-cold this batch, an auxiliary CE over ONLY the
            # pseudo-cold pool: cross-pool offsets cancel by construction, so
            # the gradient can only improve within-pool ordering (the quantity
            # our within-pool AUC instrument measures).
            if (getattr(model, "pool_ce_weight", 0.0) > 0.0
                    and getattr(model, "pseudo_cold_frac", 0.0) > 0.0):
                valid = (targets != pad_id)
                if valid.any():
                    tv = targets[valid]
                    pmask = model.pseudo_cold_mask[tv]
                    if bool(pmask.any()):
                        h_p = hidden[valid][pmask]
                        P_emb = model.item_features(model.pseudo_cold_ids)
                        logits_p = h_p @ P_emb.T
                        tgt_p = model.pseudo_cold_compact[tv[pmask]]
                        aux_p = F.cross_entropy(logits_p, tgt_p)
                        loss = loss + model.pool_ce_weight * aux_p
...
def main():
    ...
    ap.add_argument("--temporal-split", action="store_true",
                     help="PREREG_TEMPORAL_V1: replace the leave-one-out split with a "
                          "global-time split (protocol ported from the parallel session's "
                          "run_poc_temporal_lc2c_v1.py). train = t<=inner quantile; "
                          "valid = (inner, outer]; test = >outer; users need pre-cutoff "
                          "history. Cold items arise naturally (first appearance > inner).")
    ap.add_argument("--temporal-inner-q", type=float, default=0.60,
                     help="Inner (training/selection) pooled interaction-time quantile.")
    ap.add_argument("--temporal-outer-q", type=float, default=0.75,
                     help="Outer (reporting) pooled interaction-time quantile.")
    ap.add_argument("--content-anchor", action="store_true",
                     help="PREREG_TRAINED_V1 T1: base = text projection + norm-"
                          "clipped trainable delta ('Let It Go'-style, credited).")
    ap.add_argument("--delta-max", type=float, default=0.1,
                     help="T1: max L2 norm of the trainable delta row.")
    ap.add_argument("--pseudo-cold-frac", type=float, default=0.0,
                     help="T2a/T2b: fraction of items replaced by their text "
                          "projection during training (DropoutNet-style, "
                          "resampled each epoch).")
    ap.add_argument("--gbce-t", type=float, default=-1.0,
                     help="PREREG_LOSS_V1: enable gBCE (gSASRec, credited) with "
                          "calibration parameter t in [0,1]; t=1 is plain BCE. "
                          "Negative (default) disables and keeps full softmax.")
    ap.add_argument("--gbce-negs", type=int, default=256,
                     help="gBCE negatives per positive (paper uses many).")
    ap.add_argument("--pool-ce-weight", type=float, default=0.0,
                     help="T2b: weight of the within-pseudo-cold-pool CE aux "
                          "(the offset-invariant metric as a loss).")
    ...
    ap.add_argument(
        ...
        choices=["off", "identity", "learned", "fixed_ma", "fixed_hp",
                 "shared", "grouped", "lowrank", "nonlinear", "pointwise"],
        default="off",
    ...
    ap.add_argument("--fir-control-groups", type=int, default=8,
                    help="Number of channel groups for the grouped FIR arm.")
    ap.add_argument("--fir-control-rank", type=int, default=4,
                    help="Tangent rank R for the low-rank FIR arm; the realized "
                         "channel-by-lag matrix has rank at most 2R.")
    ...
    ap.add_argument("--split-dir", default=None,
                    help="Optional directory containing <category>.{train,valid,test}.csv; "
                         "default preserves the Amazon 5-core path.")
    ...
    ap.add_argument("--sequester-test-load", action="store_true",
                    help="Stronger prospective boundary: with --no-test-eval, "
                         "do not open or hash TEST bytes and construct maps from "
                         "TRAIN+VALID only. Requires a preprocessor that proves "
                         "all TEST users/items are already training-observed.")
    ...
    if args.sequester_test_load and not args.no_test_eval:
        ap.error("--sequester-test-load requires --no-test-eval")
    ...
    if args.sequester_test_load:
        # Prospective campaigns can now sequester TEST at the file-read level,
        # not merely suppress its metric.  Their deterministic preprocessors
        # must guarantee that every TEST user/item is already represented in
        # TRAIN+VALID, so the final evaluator reconstructs the identical map.
        test_rows = []
        print("  TEST SEALED: training did not read or hash test CSV bytes")
    else:
    ...
    # ---- GLOBAL-TIME PROTOCOL (PREREG_TEMPORAL_V1) -----------------------
    # Protocol design ported from the parallel session's
    # run_poc_temporal_lc2c_v1.py (pooled interaction-time quantile cutoffs;
    # first observed interaction as catalog-arrival proxy; cold = first
    # appearance after the cutoff). Their protocol, our sequential backbone.
    #
    # Replaces the leave-one-out split entirely:
    #   train = pooled events with t <= T_inner
    #   valid = events in (T_inner, T_outer]   -> best-epoch selection only
    #   test  = events >  T_outer              -> reporting
    # Item index space is UNCHANGED (reindex already covered every item), so
    # cached text embeddings stay aligned. Users need pre-cutoff history to be
    # evaluable; users without it are dropped rather than scored as cold users.
    if args.temporal_split:
        pooled = train_inters + valid_inters + test_inters
        _ts = np.array([t for (_, _, _, t) in pooled], dtype=np.int64)
        t_in = int(np.quantile(_ts, args.temporal_inner_q, method="nearest"))
        t_out = int(np.quantile(_ts, args.temporal_outer_q, method="nearest"))
        _tr = [e for e in pooled if e[3] <= t_in]
        hist_users = {e[0] for e in _tr}
        _va = [e for e in pooled if t_in < e[3] <= t_out and e[0] in hist_users]
        _te = [e for e in pooled if e[3] > t_out and e[0] in hist_users]
        # first appearance over the POOLED stream = catalog-arrival proxy
        first_seen = {}
        for (_, i, _, t) in pooled:
            if i not in first_seen or t < first_seen[i]:
                first_seen[i] = t
        n_cold = sum(1 for i, t in first_seen.items() if t > t_in)
        cold_te = sum(1 for e in _te if first_seen[e[1]] > t_in)
        # trainer-internal eval is one-target-per-user (its evaluate() signature);
        # keep each user's EARLIEST event in the window for determinism. The
        # per-event reporting eval lives in poc_temporal_eval.py.
        def _one_per_user(evs):
            best = {}
            for e in evs:
                if e[0] not in best or e[3] < best[e[0]][3]:
                    best[e[0]] = e
            return list(best.values())
        train_inters, valid_inters, test_inters = (
            _tr, _one_per_user(_va), _one_per_user(_te))
        print(f"  TEMPORAL SPLIT q_inner={args.temporal_inner_q} "
              f"q_outer={args.temporal_outer_q}: train {len(_tr):,} inters "
              f"({100*len(_tr)/len(pooled):.0f}%), val {len(valid_inters):,} users, "
              f"test {len(test_inters):,} users")
        print(f"    cold items (first appearance > inner cutoff): {n_cold:,} "
              f"({100*n_cold/max(n_items,1):.1f}% of catalog); "
              f"cold targets in raw test window: {cold_te:,}/{len(_te):,} "
              f"(pi_t = {100*cold_te/max(len(_te),1):.1f}%)")
        print(f"    NOTE 5-core was established globally; under a time cut some "
              f"items/users fall below 5 pre-cutoff interactions (declared "
              f"limitation, per prereg S2)")
    ...
    model = SASRecSBERT(n_items=n_items, pad_id=pad_id,
                         ...
                         filter_kernel=args.filter_kernel,
                          ...
                          fir_control_groups=args.fir_control_groups,
                          fir_control_rank=args.fir_control_rank,
    ...
    n_trainable_params = sum(p.numel() for p in model.parameters()
                             if p.requires_grad)
    fir_control_trainable_params = sum(
        p.numel() for name, p in model.named_parameters()
        if name.startswith("fir_control_") and p.requires_grad)
    ...
    model.content_anchor = args.content_anchor
    model.delta_max = args.delta_max
    model.pseudo_cold_frac = args.pseudo_cold_frac
    model.pool_ce_weight = args.pool_ce_weight
    model.gbce_t = args.gbce_t
    model.gbce_negs = args.gbce_negs
    if args.gbce_t >= 0.0:
        _a = args.gbce_negs / max(n_items - 1, 1)
        _b = _a * (args.gbce_t * (1.0 - 1.0 / _a) + 1.0 / _a)
        print(f"  gBCE ENABLED (gSASRec, credited): t={args.gbce_t} "
              f"K={args.gbce_negs} alpha={_a:.6f} beta={_b:.4f} "
              f"(full-softmax path disabled)")
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        if args.pseudo_cold_frac > 0.0:
            # resample the pseudo-cold pool each epoch (uniform over catalog)
            _g = torch.Generator().manual_seed(args.seed * 1000 + epoch)
            _n_p = max(2, int(round(args.pseudo_cold_frac * n_items)))
            _ids = torch.randperm(n_items, generator=_g)[:_n_p].to(DEVICE)
            _mask = torch.zeros(n_items + 1, dtype=torch.bool, device=DEVICE)
            _mask[_ids] = True
            _compact = torch.full((n_items + 1,), -1, dtype=torch.long,
                                  device=DEVICE)
            _compact[_ids] = torch.arange(_n_p, device=DEVICE)
            model.pseudo_cold_ids = _ids
            model.pseudo_cold_mask = _mask
            model.pseudo_cold_compact = _compact
    ...
    provenance = {
        ...
        "data_sha256": {
            ...
            "test_csv": (None if args.sequester_test_load else _sha256(test_csv)),
    ...
    elif getattr(model, "fir_control_group_kernels", None) is not None:
        _cw = model.fir_control_group_kernels.detach().cpu()
        fir_control_final_l2 = float(_cw.norm().item())
        fir_control_lag_profile = [
            float(v) for v in _cw.abs().mean(dim=0)]
        fir_control_profile_axis = "lag"
    elif getattr(model, "fir_control_lowrank_left", None) is not None:
        _cw = (
            model.fir_control_lowrank_left
            @ model.fir_control_lowrank_v0
            + model.fir_control_lowrank_u0
            @ model.fir_control_lowrank_right).detach().cpu()
        fir_control_final_l2 = float(_cw.norm().item())
        fir_control_lag_profile = [
            float(v) for v in _cw.abs().mean(dim=0)]
        fir_control_profile_axis = "lag"
    ...
    out = {
        ...
        "fir_control_trainable_params": fir_control_trainable_params,
        "peak_cuda_memory_bytes": (
            int(torch.cuda.max_memory_allocated())
            if torch.cuda.is_available() else None),
        "total_wall_time_s": float(time.time() - run_t0),
