...
- Results of record: `_bestrec_run/results_SOTACONF_V2_*.json` (+ tracked sidecars +
  ...
  provenance), `_bestrec_run/results_Software_FIRPROSPV3_*` (16 prospective
  matched-initialization training records plus sealed evaluations), and the per-table
  source families enumerated in `_bestrec_run/hstu_results_manifest.json` (20 required claim families)
- Table generation (fail-closed): `_bestrec_run/build_hstu_tables.py` regenerates every
  ...
  (invariants: 0 mismatch / 0 untraceable / all 20 required families sourced; the authoritative cell count is the strict build's own output — 196 at this writing)
...
6. **Prospective Software validation (PREREG_FIR_PROSPECTIVE_SW_V3; verdict SW-V3-PRACTICAL-POS):** learned−identity is +0.005062, ordinary paired 95% CI [+0.004591,+0.005533], paired t(7)=25.39, p=3.75×10⁻⁸, with 8/8 positive differences; the CI lower bound exceeds the frozen +0.000500 practical threshold. Training used validation-only checkpoint selection and no TEST scoring, followed by one sealed TEST evaluation per checkpoint and immediate committed first-reader adjudication. Scope remains prospective same-investigator, same-code-lineage, same-Amazon-family evidence under local same-user custody, not independent confirmation or cross-domain replication. Artifacts: results_Software_FIRPROSPV3_*, fir_prospective_sw_v3_{attempt,ready,endpoints_complete,status,adjudication}.json.
