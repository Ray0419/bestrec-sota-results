...
## Current build state (2026-07-31; supersedes older page-count/target notes below)

The TORS package now has three mandatory targets generated by both `build.ps1` and
`build.sh`:

- `main.tex` -> `PAPER_TORS.pdf`: focused single-column review rendering, 32 pages;
- `main-acmsmall.tex` -> `PAPER_TORS_acmsmall.pdf`: focused journal-layout main paper,
  33 pages and therefore within TORS's usual 20--35-page guidance;
- `supplement.tex` -> `PAPER_TORS_SUPPLEMENT.pdf`: 18-page reviewer supplement containing
  the extended secondary-study record, superseded/VOID histories, probe/titration ledgers,
  diagnostics, and appendix tables removed from the focused main article.

The main/supplement split changes presentation scope, not evidence or verdicts. The main
paper remains self-contained for the modular FIR claim and its negative boundary evidence;
the canonical reader retains the full record. The build keeps `main-acmsmall.aux` long enough
for the supplement's external main-paper references, then fail-closes on completion markers,
unresolved references, required supplement content, hygiene, and the acmsmall page range.
Main and supplement hygiene reports are retained separately as `hygiene_scan_output.txt` and
`hygiene_scan_supplement_output.txt`.
All 33 main pages and all 18 supplement pages were rendered and visually inspected. Real
author/affiliation/contact metadata remains a human-owned submission blocker and is accepted
only under an explicitly logged draft waiver.

The dated material below is retained as an append-only implementation history. Statements
there that call the acmsmall target untracked, describe only two targets, or give older page
counts are historical and superseded by this section.
