...
\item
  The two added components --- the \textbf{strictly causal FIR temporal filter} (confirmed on four categories, two via the pre-registered breadth campaign of \S\ref{sec:5.2}) and \textbf{TAPE} --- plus the \textbf{dataset-conditional long-tail pattern} (\S\ref{sec:5.3}) and its \textbf{interaction-thinning titration} double result (\S\ref{sec:5.4}). The novelty boundary for each is stated explicitly in Table 0 (\S\ref{sec:2.3}).
\end{itemize}
