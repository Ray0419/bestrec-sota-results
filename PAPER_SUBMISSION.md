...
*Provenance note (artifact-graph audit): the three SASRec-variant rows above are v1-era results whose per-seed artifacts were not retained; they are marked UNTRACEABLE in `hstu_results_manifest.json` and carry no claim weight. The protocol-parity argument does not rest on them: it is independently supported by the floor checks (our plain SASRec below published SASRec) and the dataset-identity table.*
