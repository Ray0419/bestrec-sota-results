...
Our base model, \textbf{SASRec-SBERT}, follows \citet{kang2018sasrec} with a frozen-text item-feature augmentation:
...
The item feature passed into the Transformer encoder is $f_i = e_i + \Phi(s_i)$. Sequences are right-padded with a dedicated pad token and processed with a causal-only attention mask (no key-padding mask --- see \S\ref{sec:3.4}).

We use a 2-layer Transformer encoder (d=64, n\_heads=2, FFN dim=4d, GELU activation, pre-norm) followed by a final LayerNorm. The output logits for position $t$ are $h_t \cdot{}$\texttt{all\_items.T} where \texttt{all\_items} is the (n\_items, d) item-features table.
...
We score each user against the \textbf{full catalog} of 207k items (Beauty\_and\_PC) or 25k items (Video\_Games). For the held-out test item, we compute the rank under dot-product scoring (after masking train+val items the user has already seen) and report NDCG@10, HR@10, MRR. We use the standard SASRec test-time convention of including the validation item in the input sequence \citep{kang2018sasrec}.
