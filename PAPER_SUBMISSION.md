...
We do **not** match the published HSTU-BLaIR Video_Games result (0.0760); we could not isolate the cause of that residual gap: the official end-to-end stack is not executable on our hardware (the pinned CUDA/Triton kernels have no sm_120 images — proven via WSL), which blocks a faithful reproduction, so kernel numerics, training recipe, feature pipeline, and other system differences remain unseparated. We therefore frame the Video_Games result as a competitive, fully-attributed, multi-seed contribution rather than a SOTA claim.
...
- **Generalization across AR2023 categories**: the confirmed results cover Video_Games (competitive, no SOTA claim), Musical_Instruments (approved point-estimate confirmation), Office_Products (provisional; VOID under the prereg floor check), and Beauty_and_PC (tail-pattern null point). Other categories untested.
...
- **Office_Products status.** Office gate values pass numerically on both kernels but the pre-registered floor check failed (+44%); the second-category pass is VOID under the prereg pending a matched comparator-baseline explanation, and no claim counts Office (§5.2).
...
- Liu, 2025. HSTU-BLaIR: Lightweight Contrastive Text Embedding for Generative Sequential Recommendation. arXiv:2504.10545. *(the external AR2023 5-core reference family: Video_Games 0.0760, Musical_Instruments 0.0406, Office_Products 0.0271; end-to-end stack not executable on our hardware)*
