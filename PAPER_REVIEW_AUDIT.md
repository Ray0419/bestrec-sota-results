...
- Branch/HEAD after fetching remote refs: `codex/bestrec-sota-results` /
  `3877b3c8` (`Respond to audit run 2026-07-19 08:32 (v1.1.9 cut with
  content-linted README; v1.1.8 marked not-for-DOI)`).
- Current deposit tag: `v1.1.9-deposit` / `44b6429e`.
- Diff from `v1.1.9-deposit` to HEAD: `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`
  only. This is response/documentation drift, not manifest-scoped empirical
  input.
- Working tree before this audit edit: no tracked modifications. Known
  untracked files: `_bestrec_run/impact_program.DONE`,
...
- Canonical sources/artifacts inspected: `PAPER_SUBMISSION.md`,
  ...
  `paper_tex/acmart.cls`, `paper_tex/hygiene_scan_output.txt`,
  `paper_tex/BUILD_NOTES.md`, `paper_tex/references.bib`, `README.md`,
  `CANONICAL_SUBMISSION.md`, `DOI_DEPOSIT_INSTRUCTIONS.md`,
  `COVER_LETTER_TORS.md`, `VENUE_PLAN.md`, `RELEASE_MANIFEST.json`,
  `_bestrec_run/rebuild_hstu_submission.py`,
  `_bestrec_run/update_release_manifest.py`,
  `_bestrec_run/build_deposit_bundle.py`, local `_release/` zips, and GitHub
  release metadata for `v1.1.8-deposit`/`v1.1.9-deposit`.
- This run edited only `PAPER_REVIEW_AUDIT.md` and the automation memory
  outside the workspace.

### Verdict

The major 08:32 rejection risk is closed: `v1.1.9-deposit` exists on GitHub,
the old `v1.1.8` release is explicitly marked superseded/not final-DOI
material, the new bundle README states the counted Office V3 and FIR-breadth
gate steps, and the builder now lints the README text so that defect should not
recur silently.

No numerical or artifact-graph blocker was found. The strict rebuild passes at
current HEAD, and the release manifest verifies against both HEAD and the
`v1.1.9-deposit` tag. The remaining top-journal issues are pre-submission
readiness rather than result validity: direct SILLM4Rec protocol inspection,
current ACM/TORS template and upload-policy verification, cover-letter fields,
and small DOI-facing metadata drift (`65` vs `66` zip entries; UTC/local release
date wording).

### Commands And Evidence Checked

- `git fetch origin --tags`; `git log --oneline -6 --decorate`;
  `git rev-parse HEAD origin/codex/bestrec-sota-results v1.1.9-deposit`;
  `git diff --name-status v1.1.9-deposit..HEAD`
  - Current HEAD and origin both resolve to `3877b3c8`.
  - `v1.1.9-deposit` resolves to `44b6429e`.
  - The only post-tag file is `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`.
- `gh release view v1.1.9-deposit --json ...`
  - Release URL:
    https://github.com/Ray0419/bestrec-sota-results/releases/tag/v1.1.9-deposit
  - Uploaded `bestrec_deposit_v1.1.9.zip` digest:
    `sha256:0998423dd1e3406582e2ea07f9cb83deff254cc6950956fbf33579c0b9d8c418`.
  - Uploaded assets include the zip, sidecar, `PAPER_SUBMISSION.pdf`,
    `PAPER_TORS.pdf`, and `RELEASE_MANIFEST.json`.
- `gh release view v1.1.8-deposit --json name,body,url`
  - Release body now says `SUPERSEDED by v1.1.9-deposit` and `Do not use as
    the final DOI artifact`.
- `Get-FileHash _release\bestrec_deposit_v1.1.9.zip`;
  downloaded sidecar comparison
  - Local zip hash matches both local and downloaded sidecar:
    `0998423dd1e3406582e2ea07f9cb83deff254cc6950956fbf33579c0b9d8c418`.
- Zip inspection via `System.IO.Compression.ZipFile`
  - `bestrec_deposit_v1.1.9.zip` has `66` entries.
  - Its bundled `README_DEPOSIT.txt` includes the full strict-chain prose:
    parity, strict table build, manifest verification, MI V2, counted Office
    V3, counted FIR-breadth, then Office V1 descriptive/VOID.
  - Its bundled `DOI_DEPOSIT_INSTRUCTIONS.md` still says `65 entries`.
- `uv --project _bestrec_run run python _bestrec_run\build_deposit_bundle.py --check-only`
  - PASS: consistency gate OK for `v1.1.9`.
  - PASS includes the new bundle-content linter.
- `uv --project _bestrec_run run python _bestrec_run\rebuild_hstu_submission.py --strict --verify-git HEAD`
  - PASS.
  - HSTU parity exact (`0.000e+00` max diff in asserted stages).
  - `168` cells recomputed; `149` exact; `19` within-rounding; `0`
    mismatches; `0` untraceable; all `14` declared claim families sourced.
  - Release-manifest verification OK for `153` files.
  - MI V2 gate OK; Office V3 adjudication OK; FIR-breadth adjudication OK;
    Office V1 remains descriptive/VOID.
- `uv --project _bestrec_run run python _bestrec_run\update_release_manifest.py --verify-git v1.1.9-deposit`
  - PASS: `128` git-backed entries match the tag's blobs exactly.
- `uv --project _bestrec_run run python paper_tex\scan_pdf.py paper_tex\PAPER_TORS.pdf`
  - PASS: `40` pages, `0` placeholder/forbidden failures, `20`
    informational SOTA/negated-claim review hits.
...
- `rg "outcome pending|no claim counts Office|not part of any counted claim|Office never a passed category|V3 outcome is pending|TODO|TBD|PLACEHOLDER|official reproduction|state-of-the-art|SOTA" ...`
  - No stale Office-pending/no-claim phrases, TODO/TBD placeholders, or
    "official reproduction" claims found in live submission sources.
  - SOTA hits are negated/non-claim contexts.

### External Fact-Check / Novelty Notes

- CTAN currently lists `acmart` version `2.19` dated `2026-06-27`, while the
  local vendored class is `v2.03` dated `2024/02/04`. Source:
  https://ctan.org/pkg/acmart
- The acmart GitHub README says the GitHub version is development/experimental
  and the production version is on CTAN and ACM sites. Source:
  https://github.com/borisveytsman/acmart
- The SILLM4Rec public repository directs users to AR2023 `5 core` files and
  describes image-description generation, user preference summaries, candidate
  product ranking tasks, and SFT/DPO training data. This supports the paper's
  repository-based non-comparability rationale, but does not replace full-text
  protocol inspection. Source: https://github.com/MKC-Lab/SILLM4Rec
- ACM/search metadata identifies SILLM4Rec as an MMAsia 2025 paper with DOI
  `10.1145/3743093.3771011`; ResearchGate reports no full text available, and
  direct ACM author/PDF fetch attempts returned HTTP 403 in this environment.
...
- TORS' 2026 methodology-policy coverage emphasizes full experimental
  pipelines, baseline tuning, preprocessing/splits/metrics, random seeds,
  dependencies, hardware, and clear limitations for unavailable artifacts.
  The paper is aligned with this direction; the sidecar and template caveats
  are the remaining presentation risks. Sources:
  https://recommender-systems.com/news/2026/04/28/acm-tors-new-policy/ and
  https://scholars.hkbu.edu.hk/en/publications/improving-methodological-standards-in-recommender-systems-offline/
- ACM's TORS launch page confirms journal fit for recommender algorithms,
  system implementations, evaluation methods, reproducibility papers, and
  registered reports. Source:
  https://www.acm.org/media-center/2022/march/tors-launch

### Confirmed Problems

1. **DOI-facing entry count is wrong.** The `v1.1.9` zip has `66` entries, but
   `DOI_DEPOSIT_INSTRUCTIONS.md` and the bundled copy still say `65 entries`.
2. **Release-date wording is ambiguous.** The GitHub release body says
   `v1.1.9` is a `2026-07-19` cut, while `_bestrec_run/build_deposit_bundle.py`
   sets `DATE = "2026-07-18"` and the internal README prints that date. This
   may be UTC-vs-Sydney dating, but DOI-facing metadata should make the basis
   explicit or align.
3. **Untracked raw-data/byproduct files are not ignored.** The current builder
   avoids them, but they remain a local release-hygiene hazard if someone uses
   a broad manual packaging or `git add` workflow.

### Plausible Risks Requiring Author Verification

- TORS double-anonymous and review-line-number policy could not be confirmed
  from a fetchable official author-guidelines page; ACM TORS guideline pages
  returned 403. Before upload, verify this in the submission portal or with the
  venue instructions.
- SILLM4Rec may still prove comparable in the full paper despite repository
  evidence suggesting a candidate-ranking/SFT-DPO protocol. Use institutional
  access before freeze if available.
- The cover letter's artifact sentence could still be read as saying the
  deposit zip contains the full live audit chain. The repository does; the zip
  deliberately contains only core historical audit documents.

### Confirmed Non-Problems

- The previous `v1.1.8` stale-bundle issue is closed by a real `v1.1.9`
  release with matching uploaded digest and supersession note.
- The executable strict gate is current: it includes counted Office V3 and
  FIR-breadth adjudicators and passes.
- The release manifest describes both HEAD and `v1.1.9-deposit` correctly under
  the documented tag-blob verification rule.
- The live manuscript sources no longer contain stale Office V3 pending/no-claim
  wording.
- The SILLM4Rec sentence is now cited and cautious; the remaining issue is lack
  of direct full-text inspection, not an overclaim.

### Concrete Fixes To Make Next

1. Change `DOI_DEPOSIT_INSTRUCTIONS.md` from `65 entries` to the actual `66`
   entries, then rebuild the deposit bundle if this file remains bundled.
2. Decide and document the release-date convention for `v1.1.9` (UTC release
   date vs Australia/Sydney audit date), and align `DATE`, release notes, and
   citation/deposit metadata before DOI minting.
3. Add ignore/preflight protection for the four recurring untracked raw/run
   byproducts, or document why they remain intentionally unignored.
4. At submission freeze, verify current TORS author instructions for
   `acmart.cls`, anonymous mode, line numbers, and first-page metadata; rebuild
   with the selected class/toolchain and rerun hygiene.
5. Obtain and inspect the SILLM4Rec ACM full text if possible; otherwise keep
   the current evidence-limited caveat.
6. Finalize `COVER_LETTER_TORS.md` bracketed fields and tighten the repository
   vs deposit wording in the data/artifacts bullet.

### Open Questions

- Should v1.1.9's release date be recorded in UTC (`2026-07-18`) or local
  audit/release time (`2026-07-19 Australia/Sydney`) across all metadata?
- Should the untracked AR2023 raw archives under `data_raw_proper/` be ignored
  explicitly to prevent accidental redistribution?
- Does TORS require or discourage review line numbers for journal submissions
  in the current ScholarOne/ACM portal?
- Can the authors access SILLM4Rec's ACM PDF through institutional credentials?

### Running Checklist

- [x] Read automation memory and prior cumulative audit.
- [x] Fetch current remote tags/branch and identify the current HEAD/tag.
- [x] Locate canonical manuscript, TeX, PDF, release, bibliography, and result
      artifacts.
- [x] Verify `v1.1.9-deposit` release existence, asset list, and zip digest.
- [x] Inspect bundled `README_DEPOSIT.txt` and DOI instructions.
- [x] Re-run strict empirical rebuild at current HEAD.
- [x] Verify manifest against `v1.1.9-deposit`.
- [x] Re-run TORS PDF hygiene scan and page-count checks.
- [x] Search live sources for stale Office wording, placeholders, and forbidden
      overclaim phrasing.
- [x] Fact-check acmart, SILLM4Rec, and TORS methodology/fit sources.
- [x] Update current prioritized rejection-risk list.
- [x] Append this timestamped audit section.
- [ ] Fix DOI/deposit entry-count and release-date metadata drift.
- [ ] Verify TORS upload/template policy at freeze.
- [ ] Inspect SILLM4Rec full paper or keep the exclusion explicitly caveated.

## Audit Run - 2026-07-19 08:32 Australia/Sydney
...
- Automation memory read from
  `C:\Users\rayxc\.codex\automations\hourly-strict-paper-audit\memory.md`.
- Branch/HEAD: `codex/bestrec-sota-results` / `9adca7fa`
  (`Respond to audit run 2026-07-19 06:27 (audit-chain wording made literal;
  403 attempts recorded)`).
- Current deposit tag: `v1.1.8-deposit` / `72749c05`.
- Diff from `v1.1.8-deposit` to HEAD:
  `COVER_LETTER_TORS.md`, `DOI_DEPOSIT_INSTRUCTIONS.md`,
  `PAPER_REVIEW_AUDIT.md`, `RELEASE_MANIFEST.json`,
  `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`, `VENUE_PLAN.md`, and
...
- Working tree before this audit edit: no tracked modifications. Known
  ...
  `data_raw_proper/cds_vinyl/CDs_and_Vinyl.csv.gz`, and
  `data_raw_proper/industrial_sci/Industrial_and_Scientific.csv.gz`.
- Canonical manuscript/artifacts inspected: `PAPER_SUBMISSION.md`,
  `PAPER_SUBMISSION.pdf`, `paper_tex/PAPER_TORS.pdf`,
  `paper_tex/PAPER_TORS_acmsmall.pdf`, `paper_tex/sections/*`,
  `paper_tex/acmart.cls`, `paper_tex/hygiene_scan_output.txt`, `README.md`,
  `CANONICAL_SUBMISSION.md`, `DOI_DEPOSIT_INSTRUCTIONS.md`,
  `COVER_LETTER_TORS.md`, `VENUE_PLAN.md`, `RELEASE_MANIFEST.json`,
  `_bestrec_run/rebuild_hstu_submission.py`,
  `_bestrec_run/update_release_manifest.py`,
  `_bestrec_run/build_deposit_bundle.py`, and the local/released
...
No empirical rejection blocker was found. The current strict gate passes at
HEAD, including the counted Office V3 and FIR-breadth adjudicators, and
manifest verification passes against both HEAD and the `v1.1.8-deposit` tag.
The manuscript PDFs remain clean of stale Office-pending/no-claim phrases and
TODO/TBD placeholders.

The new confirmed rejection risk is deposit-documentation drift. The response
to the 06:27 audit narrowed source wording and chose not to re-bundle the live
hourly audit pair, but the already-published `v1.1.8` zip still carries the old
internal README/DOI text. A reviewer using the actual DOI upload candidate sees
the bundle claim "audit chain" while the live hourly audit/response pair is not
inside the zip. The same internal README also describes the strict verification
chain without the counted Office V3 and FIR-breadth adjudicator gates, although
the executable gate does run them. This is not a result failure; it is an
artifact-readiness failure.

My top-journal recommendation: do not use `bestrec_deposit_v1.1.8.zip` as the
final DOI artifact without either a new deposit cut (`v1.1.9`) or an explicit
release-note erratum that tells reviewers to rely on the repository/tag for the
live audit pair and on the current source README/CANONICAL for the strict-chain
description.

### Commands And Evidence Checked

- `Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz'`
  - Run time: `2026-07-19 08:32:09 +10:00`.
- `git status --short`; `git log --oneline --decorate -8`;
  `git diff --name-status v1.1.8-deposit..HEAD`
  - Current HEAD: `9adca7fa`.
  - Current deposit tag: `v1.1.8-deposit` at `72749c05`.
  - No tracked working-tree changes before this audit edit.
  - Post-tag diff contains the seven support/audit files listed above, not
    only audit/response files.
- `uv --project _bestrec_run run python _bestrec_run\rebuild_hstu_submission.py --strict`
  ...
  - MI V2 gate OK.
  - Counted Office V3 adjudication OK (`CAMPAIGN VERDICT: PASS`).
  - Counted FIR-breadth adjudication OK (`Industrial_and_Scientific:
    CONFIRMED`, `CDs_and_Vinyl: CONFIRMED`).
  - Office V1 adjudication remains descriptive/VOID.
...
- `uv --project _bestrec_run run python _bestrec_run\build_deposit_bundle.py --check-only`
  - PASS in rebuild mode; no bundle written.
  - Important limitation: this check does not prove that the already-published
    zip contains the post-tag source wording.
- Structured manifest-scope check over the seven post-tag changed files
  - None of the seven files changed since `v1.1.8-deposit` is a structured
    `RELEASE_MANIFEST.json` entry. This explains why manifest verification is
    green while branch/deposit support-doc drift still exists.
- `Get-FileHash _release/bestrec_deposit_v1.1.8.zip`;
  ...
  - GitHub release asset digest is the same.
  - Release published `2026-07-18T18:30:43Z`; assets include the zip, sidecar,
    `PAPER_SUBMISSION.pdf`, `PAPER_TORS.pdf`, and `RELEASE_MANIFEST.json`.
- Zip content inspection of `_release/bestrec_deposit_v1.1.8.zip`
  - Includes `README_DEPOSIT.txt`, `DOI_DEPOSIT_INSTRUCTIONS.md`, `README.md`,
    `RELEASE_MANIFEST.json`, `VENUE_PLAN.md`,
    `_bestrec_run/adjudicate_office_v3.py`,
    `_bestrec_run/adjudicate_fir_breadth.py`, and
    `_bestrec_run/update_release_manifest.py`.
  - Does not include `PAPER_REVIEW_AUDIT.md` or
    `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`.
  - Bundled `DOI_DEPOSIT_INSTRUCTIONS.md` still says the deposit bundle
    includes the "audit chain."
  - Bundled `README_DEPOSIT.txt` still says the bundle contains "audit chain."
  - Bundled `README_DEPOSIT.txt`'s verification-chain prose names MI V2 and
    Office V1 but omits counted Office V3 and FIR-breadth adjudicators.
- Source text inspection
  - Current `README.md` and `CANONICAL_SUBMISSION.md` correctly describe the
    strict chain including Office V3 and FIR-breadth gates.
  - Current `_bestrec_run/build_deposit_bundle.py` README template still omits
    those two counted adjudicator gates.
  - Current `DOI_DEPOSIT_INSTRUCTIONS.md` narrows the bundle wording to "core
    historical audit documents" and states the live hourly audit chain is
    git-tracked rather than re-bundled per cut.
- PDF text/page check with project Python and `pypdf`
  - `paper_tex/PAPER_TORS.pdf`: 40 pages; no TODO/TBD; no stale Office
    pending/no-claim phrases; 2 SILLM4Rec mentions.
  - `paper_tex/PAPER_TORS_acmsmall.pdf`: 42 pages; same clean stale-phrase and
    placeholder result; 2 SILLM4Rec mentions.
  - `PAPER_SUBMISSION.pdf`: 46 pages; same clean stale-phrase and placeholder
    result; 1 SILLM4Rec mention.
- `paper_tex/hygiene_scan_output.txt`
  - PASS on `PAPER_TORS.pdf`, 40 pages, 0 placeholder/forbidden-claim
    failures, 20 informational SOTA/negated-claim review hits.
- `Select-String paper_tex/acmart.cls`
  - Local vendored class is `2024/02/04 v2.03`.

### External Fact-Check / Novelty Notes

- HSTU-BLaIR remains the right external point-estimate comparator for the
  manuscript's two counted category claims. Its arXiv/GitHub materials evaluate
  Video Games, Office Products, and Musical Instruments AR2023 subsets and list
  HSTU-BLaIR NDCG@10 values `0.0760`, `0.0271`, and `0.0406`. Sources:
  https://arxiv.org/html/2504.10545v3 and
  https://github.com/snapfinger/HSTU-BLaIR
- Amazon Reviews 2023's official site confirms the dataset's scale, 2023
  collection, rich review/metadata/link fields, and standard splitting. This
  supports only the manuscript's high-level dataset framing, not any
  paper-specific preprocessing claim. Source:
  https://amazon-reviews-2023.github.io/
- SILLM4Rec remains close enough to require caution. ACM metadata says the work
  uses three AR2023 5-core sub-datasets and NDCG@K metrics; the public
  repository describes image-description generation, user preference summaries,
  candidate-product ranking tasks, and SFT/DPO training data. This supports a
  non-comparability rationale but is not a substitute for direct full-text
  protocol inspection. Sources:
  https://dl.acm.org/doi/10.1145/3743093.3771011 and
  https://github.com/MKC-Lab/SILLM4Rec
- ACM's author page says LaTeX review manuscripts should use the latest Primary
  Article Template with `\documentclass[manuscript]{acmart}` for single-column
  review; CTAN lists `acmart` v2.19 dated 2026-06-27. Sources:
  https://www.acm.org/publications/authors/submissions and
  https://ctan.org/pkg/acmart?lang=en
- Recent related work remains dense around the paper's claimed territory:
  Latte (arXiv:2605.06331) modifies semantic-ID generation and reports NDCG@10
  gains; SID-MLP (arXiv:2605.12617) is a distilled generative recommender;
  GrIT (arXiv:2602.19728) claims sequential-rec improvements; ChronoSID
  (arXiv:2607.03918) adds temporal gap tokens to semantic-ID recommendation;
  Augment-or-Not (arXiv:2505.23053) benchmarks LLM recommenders; DiffuReason
  (arXiv:2602.09744) targets sequential recommendation with latent reasoning
  and refinement. The manuscript's cautious no-SOTA framing is still necessary.
  Sources: https://arxiv.org/abs/2605.06331,
  https://arxiv.org/abs/2605.12617, https://arxiv.org/abs/2602.19728,
  https://arxiv.org/abs/2607.03918, https://arxiv.org/abs/2505.23053,
  https://arxiv.org/abs/2602.09744

### Confirmed Problems

1. **The actual `v1.1.8` zip still has broad audit-chain wording.** Source
   docs were narrowed after the cut, but the published zip is byte-identical to
   the GitHub asset and still contains the old internal README/DOI wording.
   If uploaded to Zenodo as-is, a reviewer can reasonably ask why the live
   hourly audit pair is missing from a bundle that says it contains the audit
   chain.
2. **The actual `v1.1.8` zip's verification README under-describes the strict
   gate.** It omits the counted Office V3 and FIR-breadth adjudicator checks.
   The executable script is correct; the artifact-facing text is stale.
3. **The current bundle-builder README template still under-describes the
   strict gate.** A future `v1.1.9` cut would repeat problem 2 unless
   `_bestrec_run/build_deposit_bundle.py` is updated before building.
4. **The top risk list and prior memory understated HEAD drift.** HEAD is no
   longer only audit/response commits past the tag. It has support-doc and
   builder-template changes that are not present in the current deposit.
5. **SILLM4Rec full-text protocol inspection remains missing.** The current
   manuscript wording is caveated and defensible, but the full-text gap is a
   real novelty-screen weakness.
6. **ACM template/class drift, cover-letter placeholders, and line-number
   policy remain freeze blockers.**

### Confirmed Non-Problems

- The strict empirical gate remains green at current HEAD, including counted
  Office V3 and FIR-breadth live adjudicators.
- Manifest verification passes against both HEAD and `v1.1.8-deposit`.
- The paper PDFs contain no TODO/TBD placeholders and no stale Office
  pending/no-claim phrases.
- Current source `README.md`, `CANONICAL_SUBMISSION.md`, and
  `DOI_DEPOSIT_INSTRUCTIONS.md` are directionally correct about the strict
  chain and live-audit boundary; the defect is that the published zip and
  builder README template lag behind.
- HSTU-BLaIR comparator constants remain externally supported by primary
  comparator materials.

### Concrete Fixes To Make Next

1. Patch `_bestrec_run/build_deposit_bundle.py`'s `README` text so the canonical
   verification command says, in order: HSTU parity, strict table build,
   release-manifest verification, MI V2, counted Office V3 PASS, counted
   FIR-breadth CONFIRMED x2, Office V1 descriptive/VOID.
2. Add a bundle-content linter/check to prevent this recurrence:
   - generated or zipped `README_DEPOSIT.txt` must not contain bare "audit
     chain" without the "core historical / live hourly chain is git-tracked"
     qualifier;
   - generated or zipped `README_DEPOSIT.txt` must mention Office V3 and
     FIR-breadth in the strict verification-chain paragraph.
3. Cut a new `v1.1.9-deposit` if the artifact is going to DOI/Zenodo:
   bump version first, regenerate any necessary metadata, build, inspect the
   actual zip contents, verify the GitHub asset digest, and record the new tag.
4. If a new cut is intentionally deferred, add a release-note erratum or DOI
   instruction warning that `v1.1.8`'s internal README/DOI wording is stale and
   that branch source contains the corrected deposit-boundary language.
5. Inspect SILLM4Rec through institutional ACM access or keep the manuscript's
   exclusion explicitly marked as public-repo-evidence-only.
6. Resolve ACM class version and line-number policy at TORS freeze; update
   `acmart.cls` or document the portal-specific reason for retaining v2.03.
7. Fill all `COVER_LETTER_TORS.md` maintainer placeholders before upload and
   make its repository-vs-deposit sentence unambiguous.

### Open Questions

- Should `v1.1.9-deposit` be cut immediately for documentation hygiene, or only
  at the final DOI/submission freeze?
- Should the live hourly audit pair remain repository-only, or should future
  DOI bundles include a frozen copy despite the staleness risk?
- Should the release manifest expand to cover bundled support docs and the
  bundle builder itself, or is zip-level `SHA256SUMS.txt` plus git-tag scope the
  intended contract?
- Can SILLM4Rec full text be obtained before freeze?
- Does the final TORS upload require review line numbers, or should the upload
  PDF remove them while preserving double anonymity?

### Running Checklist

- [x] Read automation memory from the fallback Codex home path.
- [x] Located canonical manuscript, TeX, PDFs, release docs, manifest, builder,
      audit/response files, and local deposit zip.
- [x] Compared current HEAD against `v1.1.8-deposit`.
- [x] Re-ran strict empirical gate at current HEAD.
- [x] Verified release manifest against HEAD and `v1.1.8-deposit`.
- [x] Ran deposit-builder check-only.
- [x] Inspected actual published/local `v1.1.8` zip contents and internal
      README/DOI text.
- [x] Checked which post-tag changed files are manifest-scoped.
- [x] Checked PDF page counts and stale-phrase/placeholder extraction.
- [x] Checked local `acmart.cls` version and hygiene scan.
- [x] Fact-checked active novelty/template risks against HSTU-BLaIR,
      SILLM4Rec, AR2023, ACM, CTAN, and recent arXiv neighbors.
- [x] Updated the current prioritized rejection-risk list.
- [x] Prepended this timestamped audit section.
- [ ] Patch bundle README verification-chain prose.
- [ ] Add a bundle-content linter for audit-chain and strict-chain wording.
- [ ] Decide whether to cut `v1.1.9-deposit` before DOI/submission.
- [ ] Inspect SILLM4Rec full text.
- [ ] Resolve ACM template/version and line-number decisions.
- [ ] Fill TORS cover-letter placeholders.

## Audit Run - 2026-07-19 06:27 Australia/Sydney

### Audited State

- Workspace: `C:\Users\rayxc\Documents\R`.
- Automation memory: missing at start of this run; reconstructed current focus
  from `PAPER_REVIEW_AUDIT.md` and `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`.
- Branch/HEAD: `codex/bestrec-sota-results` / `7608af24`
  (`Respond to audit run 2026-07-19 04:31 (CP-1..3 already closed by the
  v1.1.8 cut, per the run's own risk list)`).
- Current deposit tag: `v1.1.8-deposit` / `72749c05`.
- Diff from `v1.1.8-deposit` to HEAD: only `PAPER_REVIEW_AUDIT.md` and
  `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`.
- Working tree before this audit edit: no tracked modifications. Known
  untracked files remain `_bestrec_run/impact_program.DONE`,
  `_bestrec_run/smoke_FIRB_IS_seed1.json`,
...
- Canonical manuscript/artifacts inspected: `PAPER_SUBMISSION.md`,
  `PAPER_SUBMISSION.pdf`, `paper_tex/PAPER_TORS.pdf`,
  `paper_tex/PAPER_TORS_acmsmall.pdf`, `paper_tex/sections/*`,
  `paper_tex/references.bib`, `paper_tex/acmart.cls`,
  `paper_tex/hygiene_scan_output.txt`, `README.md`,
  `CANONICAL_SUBMISSION.md`, `DOI_DEPOSIT_INSTRUCTIONS.md`,
  `RELEASE_MANIFEST.json`, `_bestrec_run/build_deposit_bundle.py`,
  `_bestrec_run/hstu_tables.json`, and the local
  `_release/bestrec_deposit_v1.1.8.zip`.
- This run edited only `PAPER_REVIEW_AUDIT.md` and the automation memory outside
  the workspace.

### Verdict

No new empirical rejection blocker was found. The strict artifact graph remains
green at current HEAD, and both the current branch and the `v1.1.8-deposit` tag
verify against the release manifest. The current two-commit drift past the tag
is response/audit-only and is explicitly covered by `README.md`'s statement that
the archival boundary is the deposit tag, not branch HEAD.

The new confirmed issue is narrower but reviewer-facing: the repository and DOI
instructions use broad "audit chain" language, while the actual v1.1.8 deposit
bundle omits the current hourly audit pair (`PAPER_REVIEW_AUDIT.md` and
`RESPONSE_TO_PAPER_REVIEW_AUDIT.md`). Since these files contain the live
adversarial review and response trail, omitting them while saying "full
adversarial audit chain" risks looking selective. This should be fixed in the
next deposit cut or by narrowing the wording.

SILLM4Rec remains unresolved: the manuscript's caveated exclusion is defensible
from public repository evidence, but direct ACM full-text/PDF access returned
403 in this environment. A top-journal reviewer can still ask for a full-text
protocol inspection before accepting the non-comparability statement.

### Commands And Evidence Checked

- `Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz'`
  - Run time: `2026-07-19 06:27:03 +10:00`.
- `git rev-parse --short HEAD`; `git rev-parse --short v1.1.8-deposit`;
  `git diff --name-status v1.1.8-deposit..HEAD`; `git status --short --branch`
  - `HEAD = 7608af24`; `v1.1.8-deposit = 72749c05`.
  - Diff from the deposit tag contains only `PAPER_REVIEW_AUDIT.md` and
    `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`.
  - No tracked working-tree modifications before this audit edit.
- `uv --project _bestrec_run run python _bestrec_run/rebuild_hstu_submission.py --strict`
  - PASS.
  - HSTU parity exact (`0.000e+00` max diff).
  ...
  - Release-manifest verification OK.
  - MI V2 gate PASS, counted Office V3 PASS, counted FIR breadth CONFIRMED x2,
    Office V1 descriptive adjudicator OK/VOID.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify-git HEAD`
  - PASS: `128` git-backed entries match HEAD blobs exactly.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify-git v1.1.8-deposit`
  - PASS: `128` git-backed entries match the tag blobs exactly.
- `uv --project _bestrec_run run python _bestrec_run/build_deposit_bundle.py --check-only`
  - PASS: rebuild mode; metadata versions and manifest boundary agree with
    `v1.1.8`; no bundle written.
- `gh release view v1.1.8-deposit --repo Ray0419/bestrec-sota-results --json ...`
  - Release exists, not draft, not prerelease; published
    `2026-07-18T18:30:43Z`.
  - Assets: `bestrec_deposit_v1.1.8.zip`,
    `bestrec_deposit_v1.1.8.zip.sha256`, `PAPER_SUBMISSION.pdf`,
    `PAPER_TORS.pdf`, and `RELEASE_MANIFEST.json`.
- PDF text/page check with project Python and `pypdf`
  - `paper_tex/PAPER_TORS.pdf`: 40 pages, no TODO/TBD, no stale Office
    pending/no-claim phrases, 2 SILLM4Rec mentions.
  - `paper_tex/PAPER_TORS_acmsmall.pdf`: 42 pages, same clean placeholder/stale
    phrase result.
  - `PAPER_SUBMISSION.pdf`: 46 pages, same clean placeholder/stale phrase
    result, 1 SILLM4Rec mention.
- `paper_tex/hygiene_scan_output.txt`
  - PASS on `PAPER_TORS.pdf`, 40 pages, 0 placeholder/forbidden-claim failures,
    20 informational SOTA/negated-claim review hits.
- `tar -tf _release/bestrec_deposit_v1.1.8.zip`
  - Bundle includes selected audit/response files:
    `RESPONSE_TO_CODEX_AUDIT.md`, `RESPONSE_TO_FULL_METHOD_AUDIT.md`,
    `RESPONSE_TO_NOVELTY_AUDIT.md`,
    `RESPONSE_TO_RESUBMISSION_AUDIT_2026-07-11.md`, and
    `STRICT_RESUBMISSION_AUDIT_2026-07-11.md`.
  - Bundle does **not** include `PAPER_REVIEW_AUDIT.md` or
    `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`.
- `Select-String` over `README.md` / `DOI_DEPOSIT_INSTRUCTIONS.md`
  - `README.md` says the repository contains the "full adversarial audit chain"
    and lists `PAPER_REVIEW_AUDIT.md` / `RESPONSE_TO_PAPER_REVIEW_AUDIT.md` as
    the hourly adversarial audit chain.
  - `DOI_DEPOSIT_INSTRUCTIONS.md` says the deposit bundle includes the "audit
    chain."
- SILLM4Rec ACM full/PDF access probe
  - `Invoke-WebRequest` HEAD to both the ACM full and PDF URLs returned 403
    Forbidden in this environment.

### External Fact-Check / Novelty Notes

- HSTU-BLaIR arXiv v3 supports the comparator constants and dataset-statistics
  framing used by this paper: it evaluates AR2023 Video Games, Office Products,
  and Musical Instruments 5-core subsets, lists item/user/interactions values
  matching the manuscript family, and reports HSTU-BLaIR NDCG@10 values
  `0.0760`, `0.0271`, and `0.0406` in its result table. Source:
  https://arxiv.org/html/2504.10545
- Amazon Reviews 2023's official site confirms the dataset is a 2023 McAuley
  Lab release with user reviews, item metadata, links, and standard splitting;
  this supports the paper's high-level dataset framing but not any paper-specific
  preprocessing claim by itself. Source: https://amazon-reviews-2023.github.io/
- SILLM4Rec remains close enough to require caution. ACM metadata/search snippets
  identify the DOI/title and state that experiments use three AR2023 5-core
  sub-datasets and report NDCG@1/5/10, while the public repository workflow says
  to generate image descriptions, user preference summaries, candidate product
  ranking tasks, and SFT/DPO data. That supports the manuscript's
  non-comparability rationale but does **not** substitute for full-text protocol
  inspection. Sources: https://dl.acm.org/doi/10.1145/3743093.3771011 and
  https://github.com/MKC-Lab/SILLM4Rec
- ACM's current submissions page says LaTeX review manuscripts should use the
  latest Primary Article Template and `\documentclass[manuscript]{acmart}` for
  single-column review; CTAN lists production `acmart` v2.19 dated 2026-06-27.
  The local vendored class is v2.03 dated 2024-02-04. Sources:
  https://www.acm.org/publications/authors/submissions and
  https://ctan.org/tex-archive/macros/latex/contrib/acmart?lang=en

### Confirmed Problems

1. **Deposit/audit-chain wording is broader than the actual v1.1.8 bundle.**
   The current hourly audit pair is tracked in git and described in the README
   as the hourly adversarial audit chain, but the released deposit bundle omits
   it while DOI instructions say the bundle includes the audit chain. This is
   not a numerical support failure, but it is a credibility and archival
   completeness issue.
2. **SILLM4Rec full-text protocol inspection remains missing.** The current
   manuscript wording is appropriately caveated, but a reviewer can still treat
   this as incomplete related work because ACM metadata makes it AR2023-5-core
   and metric-adjacent.
3. **ACM template/class drift remains unresolved.** Local `acmart.cls` v2.03
   trails CTAN production v2.19 and ACM's latest-template guidance.
4. **Cover-letter maintainer fields remain bracketed.** This is acceptable only
   while `COVER_LETTER_TORS.md` remains a draft.
5. **Review-line-number policy remains undecided.** The TORS review PDF remains
   in review/line-number mode; this may be correct for review but must be
   checked against the final target's upload instructions.

### Confirmed Non-Problems

- The strict empirical gate remains green at current HEAD.
- Manifest verification passes against both HEAD and `v1.1.8-deposit`.
- The two commits after `v1.1.8-deposit` are response/audit-only, and README
  explicitly states the archival boundary is the tag, not branch HEAD.
- PDF text extraction found no stale Office pending/no-claim phrases and no
  TODO/TBD placeholders in the paper PDFs.
- Current SILLM4Rec wording is cautiously evidence-limited; the problem is the
  missing full-text inspection, not an already-overstated comparison.

### Concrete Fixes To Make Next

1. Decide the audit-chain deposit boundary and make it literal:
   - If the deposit should include the live adversarial audit trail, add
     `PAPER_REVIEW_AUDIT.md` and `RESPONSE_TO_PAPER_REVIEW_AUDIT.md` to
     `_bestrec_run/build_deposit_bundle.py`, rebuild, verify, and cut
     `v1.1.9-deposit`.
   - If not, change README/DOI wording from "full adversarial audit chain" /
     "audit chain" to "selected audit documents" or "core historical audit
     chain."
2. Obtain SILLM4Rec full text through ACM/institutional access and inspect its
   exact candidate set, split, ranking universe, metrics, categories, and table
   values. If unavailable, keep the current caveat and add "full text not
   inspected; public repository evidence only" to freeze notes.
3. Refresh `paper_tex/acmart.cls` to the current ACM template or record a
   venue-specific reason for retaining v2.03.
4. Fill `COVER_LETTER_TORS.md` maintainer fields before ScholarOne upload.
5. Decide whether review line numbers are required/allowed for the final TORS
   PDF and rerender if needed.

### Open Questions

- Should `PAPER_REVIEW_AUDIT.md` and `RESPONSE_TO_PAPER_REVIEW_AUDIT.md` be
  treated as deposit payloads, or are they intentionally branch-only process
  artifacts?
- Can the SILLM4Rec ACM PDF be accessed through an institutional route before
  freeze?
- Does the final TORS target require `review` line numbers, or should the upload
  PDF remove them while preserving anonymity?
- Are local-only Office/FIR per-user sidecars acceptable as "on request / upon
  acceptance" material for this venue?

### Running Checklist

- [x] Read automation memory location; confirmed memory file was missing.
- [x] Located canonical manuscript, TeX, PDFs, references, release/deposit
      docs, manifest, build script, audit/response files, and local deposit zip.
- [x] Re-ran strict empirical gate at current HEAD.
- [x] Verified release manifest against HEAD and `v1.1.8-deposit`.
- [x] Checked deposit builder consistency gate.
- [x] Checked GitHub release metadata/assets for `v1.1.8-deposit`.
- [x] Checked PDF page counts and placeholder/stale-phrase extraction.
- [x] Checked local deposit zip contents against audit-chain wording.
- [x] Fact-checked active novelty/template risks against HSTU-BLaIR, SILLM4Rec,
      Amazon Reviews 2023, ACM submissions, and CTAN sources.
- [x] Updated the current prioritized rejection-risk list.
- [x] Appended this timestamped audit section.
- [ ] Decide whether to bundle the live hourly audit/response pair or narrow
      deposit wording.
- [ ] Inspect SILLM4Rec full text.
- [ ] Resolve ACM template/version and line-number decisions.
- [ ] Fill TORS cover-letter placeholders.
...
- Workspace: `C:\Users\rayxc\Documents\R`.
- Branch/HEAD: `codex/bestrec-sota-results` / `05d01aad`
  (`Companion quiet-tick: counted-results-have-their-own-referees line in the
  trust story`).
- Current advertised deposit tag: `v1.1.7-deposit` / `1828ed4b`.
- No workspace files were modified after the prior automation memory timestamp
  (`2026-07-19 03:33:30 +10:00`). This was therefore a stability/recheck audit,
  not a new-manuscript-delta audit.
- Working tree before this edit: `PAPER_REVIEW_AUDIT.md` already modified from
  the prior audit; known untracked files only:
...
No new empirical or manuscript-prose contradiction was found, and the strict
artifact gate remains green. The paper is still not submission-clean because the
current branch presents `v1.1.7-deposit` as the literal reviewer-facing deposit
target while the current manifest verifies only against HEAD. This is a
rejection-grade artifact packaging issue, not a results issue: a reviewer who
follows the branch documentation and runs `--verify-git v1.1.7-deposit` gets a
real mismatch.

The external spot-check also strengthens the freeze warning on SILLM4Rec. The
manuscript is right to avoid a comparison and to label the exclusion as pending
full-text inspection, but SILLM4Rec cannot be waved away from repository evidence
alone because public ACM/search metadata says it uses AR2023 5-core sub-datasets
and reports NDCG. A top-journal reviewer may ask why this close 2025 multimodal
recommendation paper was not protocol-inspected.

### Commands And Evidence Checked

- `Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz'`
  - Run time: `2026-07-19 04:25:51 +10:00`.
- `git rev-parse --short HEAD`; `git rev-parse --short v1.1.7-deposit`;
  `git status --short --branch`
  - `HEAD = 05d01aad`; tag `v1.1.7-deposit = 1828ed4b`.
  - Only `PAPER_REVIEW_AUDIT.md` was modified in tracked files, from prior audit
    work.
- File mtime check against `2026-07-19T03:33:30+10:00`
  - No workspace file newer than the last memory timestamp was found.
- `uv --project _bestrec_run run python _bestrec_run/rebuild_hstu_submission.py --strict`
  - PASS.
  - HSTU parity exact (`0.000e+00` max diff).
  - `168` cells recomputed; `149` exact; `19` within-rounding; `0` mismatches;
  ...
  - MI V2 PASS, counted Office V3 PASS, counted FIR breadth CONFIRMED x2, Office
    V1 descriptive adjudicator OK/VOID.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify-git HEAD`
  - PASS: `128` git-backed entries match HEAD blobs.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify-git v1.1.7-deposit`
  - FAIL: `1` mismatch, `127` OK.
  - Mismatch: `submission_docs/CANONICAL_SUBMISSION.md`.
- `uv --project _bestrec_run run python _bestrec_run/build_deposit_bundle.py --check-only`
  - PASS in rebuild mode: `--verify-git HEAD OK`; metadata versions and manifest
    boundary agree with `v1.1.7`.
  - This means the builder path is healthy at HEAD; it does not make the old tag
    match the current manifest.
- `gh release view v1.1.7-deposit --repo Ray0419/bestrec-sota-results --json ...`
  - Release exists, not draft, not prerelease; published
    `2026-07-18T14:31:53Z`.
  - Published release asset digests include zip
    `dd76fc1792f3060fb85de18c595d9dec50f14dd1927f8ca77a9c7b4fb9216f34`,
    manifest `3e9ff7dbbb22dface3f93a319e421ec7f9daf9ce990fbf2380abd2b41ac75052`,
    `PAPER_SUBMISSION.pdf`
    `c765e751b2c20aba06bfa591ab6b9d1004b3840c2dba2af7910aad7a45fd1531`,
    and `PAPER_TORS.pdf`
    `55a603a93acb13717b32dccbd6887f34cdc5d3e333a302becdfa509e42650fe9`.
- `rg` stale-claim/placeholder scan across `PAPER_SUBMISSION.md`,
  `paper_tex/sections`, `paper_tex/hygiene_scan_output.txt`,
  `COVER_LETTER_TORS.md`, `DOI_DEPOSIT_INSTRUCTIONS.md`, and
  `CANONICAL_SUBMISSION.md`
  - No manuscript hit for stale Office "pending" or "Office never a passed
    category" wording.
  - One `CANONICAL_SUBMISSION.md` hit for "not part of any counted claim" refers
    specifically to local-only per-user sidecars, not to Office V3 aggregate
    claims.
  - `COVER_LETTER_TORS.md` still contains maintainer-fill bracketed fields.
- PDF checks via `pypdf`
  - `PAPER_SUBMISSION.pdf`: 46 pages; final page has only `42` extracted text
    characters on one line.
  - `paper_tex/PAPER_TORS.pdf`: 40 pages; final page has normal extracted text
    volume.
  - `paper_tex/PAPER_TORS_acmsmall.pdf`: 42 pages; final page has normal
    extracted text volume.
- `paper_tex/hygiene_scan_output.txt`
  - PASS: 40 pages, `0` placeholder/forbidden failures, `20` informational
    review hits.
- `paper_tex/acmart.cls`
  - Local vendored class remains `2024/02/04 v2.03`.

### External Fact-Check / Novelty Notes

- HSTU-BLaIR comparator constants remain externally supported: the arXiv v3 HTML
  lists AR2023 Video Games, Office Products, and Musical Instruments statistics
  matching the manuscript's family, and reports HSTU-BLaIR NDCG@10 `0.0760`,
  `0.0271`, and `0.0406` respectively. Source:
  https://arxiv.org/html/2504.10545v3
- Latte's public arXiv HTML supports the manuscript's related-work numbers:
  Table 1 reports Instrument NDCG@10 `0.0331` and Games NDCG@10 `0.0515`, with
  Amazon Reviews 2023 LOO setup. Source: https://arxiv.org/html/2605.06331
- GrIT is correctly close but below the manuscript's Video_Games point estimate:
  its arXiv HTML reports Video Games NDCG@10 `0.0588` and also includes
  Industrial & Scientific and CDs & Vinyl result rows. Source:
  https://arxiv.org/html/2602.19728
- ReSID and ChronoSID support the manuscript's non-interchangeability rationale:
  their dataset statistics are `57,359 / 23,742 / 490,522` for Musical
  Instruments, not the HSTU-BLaIR-family `57,439 / 24,587 / ~511,835`. ChronoSID
  Table 6 reports output-level MI NDCG@10 `0.0345` vs ReSID `0.0325`, matching
  the manuscript's statement. Sources: https://arxiv.org/html/2602.02338 and
  https://arxiv.org/html/2607.03918
- WPGRec is correctly cited as a later frequency/time-frequency sequential
  recommendation line: arXiv says it uses wavelet-packet time-frequency modeling
  and is accepted to SIGIR 2026. Source: https://arxiv.org/abs/2604.21305
- SILLM4Rec remains a close-work risk. The public GitHub workflow describes
  image-description generation, user preference summaries, candidate product
  ranking tasks, and SFT/DPO training; that supports the paper's caution that
  the protocol may not be full-catalog LLOO. But ACM/search metadata says the
  paper conducts experiments on three AR2023 5-core sub-datasets and reports
  NDCG@1/5/10. Sources: https://github.com/MKC-Lab/SILLM4Rec and
  https://dl.acm.org/doi/full/10.1145/3743093.3771011
- ACM/CTAN template risk remains current. ACM's author page asks LaTeX review
  submissions to use the latest Primary Article Template and
  `\documentclass[manuscript]{acmart}`; CTAN lists `acmart` v2.19 dated
  `2026-06-27`, while the repo vendors v2.03. Sources:
  https://www.acm.org/publications/authors/submissions and
  https://ctan.org/pkg/acmart?lang=en

### Confirmed Problems

1. **Current manifest vs advertised tag mismatch remains.** HEAD's manifest
   verifies against HEAD but not against `v1.1.7-deposit`; the mismatch is
   `CANONICAL_SUBMISSION.md`.
2. **Deposit instructions still point at a dangerous local-upload path.**
   `DOI_DEPOSIT_INSTRUCTIONS.md` says to upload `bestrec_deposit_v1.1.7.zip`
   from the `v1.1.7` release assets or `_release/` locally. That is not safe
   after post-tag manifested-doc changes because the local rebuilt `_release/`
   bundle is HEAD-derived, while the published release sidecar is v1.1.7-derived.
3. **No deposit tag archives the current builder/doc state.** The published
   `v1.1.7` release is self-consistent, but the current branch's manifest and
   support docs have moved past it.
4. **ACM class remains stale.** Local `acmart.cls` v2.03 trails CTAN v2.19 and
   the ACM site still directs authors to the latest Primary Article Template.
5. **Cover letter remains incomplete.** Preprint status, conflicts, reviewer
   suggestions/exclusions, and author contact fields remain bracketed.
6. **SILLM4Rec has not been directly protocol-inspected.** Current exclusion is
   honestly caveated, but this remains a likely reviewer question.

### Confirmed Non-Problems

- The strict empirical gate is green.
- The previous Office V1/V3 contradiction is not present in the manuscript
  source searched in this pass.
- HSTU-BLaIR comparator constants, Latte/GrIT/ReSID/ChronoSID related-work
  statements, and WPGRec frequency-line framing were not contradicted by the
  external primary/public sources checked.
- The reader PDF's nearly blank final page does not affect `PAPER_TORS.pdf`,
  which is the TORS review artifact; treat it as cosmetic unless the reader PDF
  will be submitted or distributed.

### Concrete Fixes To Make Next

1. Cut and publish `v1.1.8-deposit` from the current HEAD state, or remove any
   branch-level wording that presents `v1.1.7-deposit` as the current literal
   reviewer target.
2. Update `DOI_DEPOSIT_INSTRUCTIONS.md` so manual-upload instructions point only
   to the release asset for the advertised tag, or to a newly cut tag, not to a
   potentially stale local `_release/` artifact.
3. Add a one-sentence SILLM4Rec freeze note: "full text not inspected; public
   repo suggests candidate-ranking/SFT-DPO protocol; ACM metadata confirms
   AR2023 5-core/NDCG, so exclusion is evidence-limited."
4. Refresh `paper_tex/acmart.cls` to the current ACM template, or record a
   venue-specific decision that v2.03 is intentionally frozen for this
   submission.
5. Fill `COVER_LETTER_TORS.md` maintainer placeholders before upload.

### Open Questions

- Should the next automation run cut `v1.1.8-deposit`, or should the branch docs
  be edited to say `v1.1.7-deposit` is only the last archived snapshot while HEAD
  is newer?
- Can the SILLM4Rec ACM PDF be accessed through an institutional route before
  freeze?
- Does TORS want visible red review line numbers in the uploaded manuscript, or
  should the final review PDF drop the `review` option?

### Running Checklist

- [x] Read automation memory.
- [x] Locate canonical manuscript, TeX, PDFs, deposit docs, release manifest,
      result/adjudicator artifacts, and prior cumulative audit.
- [x] Check whether files changed after the prior memory timestamp.
- [x] Rerun strict artifact gate.
- [x] Rerun manifest verification against HEAD and the advertised deposit tag.
- [x] Check deposit-builder consistency gate.
- [x] Check GitHub release metadata/digests for `v1.1.7-deposit`.
- [x] Search manuscript/support docs for stale Office and placeholder wording.
- [x] Check current PDF page counts and hygiene scan.
- [x] Fact-check close literature/template claims against primary/public sources.
- [x] Refresh the prioritized rejection-risk list.
- [x] Append this timestamped audit section.
- [ ] Resolve the deposit tag/branch boundary.
- [ ] Inspect SILLM4Rec full text or keep exclusion explicitly evidence-limited.
- [ ] Refresh ACM template or document a conscious freeze exception.
- [ ] Fill cover-letter placeholders before submission.

## Audit Run - 2026-07-19 03:30 Australia/Sydney

### Audited State

- Workspace: `C:\Users\rayxc\Documents\R`.
- Branch/HEAD: `codex/bestrec-sota-results` /
  `05d01aad` (`Companion quiet-tick: counted-results-have-their-own-referees line in the trust story`).
- Current published deposit tag: `v1.1.7-deposit` / `1828ed4b`.
- Commits since the last audited `00:35` state:
  - `80c2f680`: builder two-mode boundary model, deterministic zip, argparse,
    `CANONICAL` strict-chain sync, README hash rule.
  - `04864d2d`: response to the 00:35 audit.
  - `05d01aad`: plain-language companion/site wording.
- `git diff --name-status v1.1.7-deposit..HEAD` includes
  `CANONICAL_SUBMISSION.md`, `PAPER_REVIEW_AUDIT.md`,
  `PLAIN_LANGUAGE_COMPANION.md`, `README.md`, `RELEASE_MANIFEST.json`,
  `RESPONSE_TO_PAPER_REVIEW_AUDIT.md`, `_bestrec_run/build_deposit_bundle.py`,
  and `companion_site/explainer.html`.
- Initial tracked working tree was clean; known untracked files remained
  `_bestrec_run/impact_program.DONE`,
  `_bestrec_run/smoke_FIRB_IS_seed1.json`,
  `data_raw_proper/cds_vinyl/CDs_and_Vinyl.csv.gz`, and
  `data_raw_proper/industrial_sci/Industrial_and_Scientific.csv.gz`.
- This run edited only `PAPER_REVIEW_AUDIT.md` plus automation memory outside
  the workspace. Verification generated ignored temp files under `tmp/` and
  rebuilt ignored `_release/bestrec_deposit_v1.1.7.zip`.

### Verdict

The previous `00:35` blocker is partially fixed but not submission-clean. The
builder no longer false-fails at HEAD: help, check-only, and full build all
pass; the local rebuilt zip is byte-stable across repeated builds and internally
hash-clean. `CANONICAL_SUBMISSION.md` is also synchronized with the actual live
strict gate.

The new hard artifact-review blocker is the tag boundary. The current workspace
manifest still says `intended_deposit_tag = v1.1.7-deposit`, and README/CANONICAL
say that tag is the archival boundary, but the current `RELEASE_MANIFEST.json`
does **not** verify against that tag. It verifies against HEAD. A reviewer
following the literal current-branch command
`update_release_manifest.py --verify-git v1.1.7-deposit` will get a mismatch in
`CANONICAL_SUBMISSION.md`. The published `v1.1.7` GitHub release remains
self-consistent, but it is now behind the support-doc/builder state in HEAD. For
a top-journal artifact package, either cut `v1.1.8-deposit` from this state, or
make the branch stop claiming that the current manifest's literal target is the
old tag.

### Commands And Evidence Checked

- `git status --short --branch`
  - Tracked tree clean at audit start; known untracked scratch/raw-data files
    only.
...
- `uv --project _bestrec_run run python _bestrec_run/rebuild_hstu_submission.py --strict`
  ...
  - `168` cells recomputed; `149` exact; `19` within-rounding; `0` mismatches;
    `0` untraceable; all `14` declared claim families sourced.
  - Release-manifest verification OK.
  - MI V2 PASS, Office V3 PASS, FIR breadth CONFIRMED x2, Office V1 descriptive
    adjudicator OK/VOID.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify`
  - PASS: `153` files verified.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify-git HEAD`
...
- `uv --project _bestrec_run run python _bestrec_run/build_deposit_bundle.py --help`
  - PASS: argparse help prints; the old pre-help consistency failure is gone.
- `uv --project _bestrec_run run python _bestrec_run/build_deposit_bundle.py --check-only`
  - PASS: `boundary: REBUILD mode (--verify-git HEAD OK; git_commit is the
    documented parent)`.
- `uv --project _bestrec_run run python _bestrec_run/build_deposit_bundle.py`
  - PASS twice with identical SHA256:
    `18f4b3ce7bf8195c56134aa8f3bf2c9265dd3e264e6b43bc75493e29212043d0`.
  - Archive size: `1,803,558` bytes; entries: `66`.
  - Internal `SHA256SUMS.txt`: `65` payload rows, `0` mismatches.
- `gh release view v1.1.7-deposit --repo Ray0419/bestrec-sota-results`
  ...
  - Assets present: `bestrec_deposit_v1.1.7.zip`,
    ...
    `PAPER_TORS.pdf`, `RELEASE_MANIFEST.json`.
- Downloaded current GitHub release manifest/sidecar to
  `tmp/paper_audit_gh_v117_20260719_032704/`.
  - Published release manifest SHA256:
    `3e9ff7dbbb22dface3f93a319e421ec7f9daf9ce990fbf2380abd2b41ac75052`.
  - Published release zip sidecar:
    `dd76fc1792f3060fb85de18c595d9dec50f14dd1927f8ca77a9c7b4fb9216f34`.
  - Current worktree `RELEASE_MANIFEST.json` raw SHA256:
    `d37bac98544a81fadd834cbf2c5d92a692dcd5d3d6830296dd8c106f2c575b70`.
  - Local rebuilt `_release/bestrec_deposit_v1.1.7.zip` SHA256:
    `18f4b3ce7bf8195c56134aa8f3bf2c9265dd3e264e6b43bc75493e29212043d0`.
- `rg` manuscript/support text checks:
  - No live compiled-manuscript hit for stale "outcome pending" / "Office never
    a passed category" / "not part of any counted claim" contradictions.
  - `COVER_LETTER_TORS.md` still has maintainer placeholders for preprint
    status, conflicts, and suggested/excluded reviewers.
- PDF/page checks:
  - `PAPER_SUBMISSION.pdf`: `46` pages; page 46 has only one short line and is
    visually almost blank.
  - `paper_tex/PAPER_TORS.pdf`: `40` pages; hygiene scan PASS with
    `0` placeholder/forbidden failures and `20` informational review hits.
  - `paper_tex/PAPER_TORS_acmsmall.pdf`: `42` pages.
  - Rendered visual samples:
    `tmp/pdfs/paper_audit_20260719_0331/PAPER_TORS_p001.png`,
    `PAPER_TORS_p011.png`, `PAPER_TORS_p040.png`,
    `PAPER_SUBMISSION_p001.png`, `PAPER_SUBMISSION_p046.png`.
    Samples were legible, unclipped, and not visibly overlapped. TORS retains
    prominent red line numbers.
- Template check:
  - `paper_tex/acmart.cls` is `2024/02/04 v2.03`.
  - CTAN currently lists `acmart` version `2.19` dated `2026-06-27`.

### External Fact-Check / Novelty Notes

- HSTU-BLaIR v3 supports the paper's comparator constants and dataset
  statistics: Video Games `25,612 / 94,762 / 814,585`, Office Products
  `77,551 / 223,308 / 1,800,877`, Musical Instruments
  `24,587 / 57,439 / 511,835`; Table 2 reports NDCG@10 `0.0760`,
  `0.0271`, and `0.0406` for HSTU-BLaIR respectively. Source:
  https://arxiv.org/html/2504.10545v3
- The official Amazon Reviews 2023 site supports the dataset framing: a 2023
  McAuley Lab release with reviews, metadata, links, larger/newer coverage,
  and standard splitting support. Source: https://amazon-reviews-2023.github.io/
- FMLP-Rec and BSARec support the paper's "incremental over prior frequency
  filters" novelty boundary. FMLP-Rec explicitly proposes learnable filters in
  the frequency domain; BSARec uses Fourier transform to inject frequency
  inductive bias and combine low/high-frequency information. Sources:
  https://arxiv.org/abs/2202.13556 and
  https://ojs.aaai.org/index.php/AAAI/article/view/28747
- SILLM4Rec remains a plausible close-work risk, but the public repository
  supports the paper's cautious non-comparability wording: it instructs users to
  create candidate product-ranking tasks and generate SFT/DPO training data,
  not a clearly established full-catalog LLOO ranking protocol. Source:
  https://github.com/MKC-Lab/SILLM4Rec
- ACM/CTAN template check: ACM's author workflow requires single-column review
  manuscript mode; CTAN lists `acmart` v2.19 dated 2026-06-27, while the repo
  vendors v2.03. Sources:
  https://www.acm.org/publications/authors/submissions and
  https://ctan.org/pkg/acmart?lang=en

### Confirmed Problems

1. **Current manifest vs intended tag mismatch.** HEAD's manifest verifies
   against HEAD but not against `v1.1.7-deposit`; the mismatch is
   `CANONICAL_SUBMISSION.md`.
2. **Local `_release/` zip no longer matches the published `v1.1.7` sidecar.**
   This is generated/ignored, but `DOI_DEPOSIT_INSTRUCTIONS.md` still says a
   maintainer can use `_release/` locally for the current bundle.
3. **No deposit tag archives the builder/doc fix at HEAD.** The published
   `v1.1.7` release is self-consistent but lacks the post-tag builder and
   `CANONICAL` sync.
4. **ACM class remains stale.** Local `acmart.cls` v2.03 trails CTAN v2.19.
5. **Review PDFs still carry cosmetic upload risks.** TORS has red line
   numbers; reader PDF ends with an almost blank final page.
6. **Cover letter still has maintainer placeholders.**
7. **SILLM4Rec still lacks direct full-text protocol inspection.**

### Confirmed Fixes / Non-Problems

- The old builder false-failure is fixed at HEAD.
- Builder output is deterministic across repeated local builds from HEAD.
- `CANONICAL_SUBMISSION.md` now reflects the counted Office V3 and FIR-breadth
  strict-gate chain.
- The strict empirical gate is green.
- The Office V1/V3 manuscript story is now two-track and no longer appears
  internally contradictory in the checked files.
- Published `v1.1.7` GitHub release still exists with the expected assets; its
  downloaded sidecar and manifest are stable relative to the prior audit.

### Concrete Fixes To Make Next

1. Cut a new deposit release from current HEAD, likely `v1.1.8-deposit`:
   regen manifest with `--deposit-tag v1.1.8-deposit`, build once, commit once,
   tag, upload zip/sidecar/manifest/PDFs, and verify
   `--verify-git v1.1.8-deposit`.
2. Bump `_bestrec_run/build_deposit_bundle.py` `VERSION` from `v1.1.7` to the
   new cut before building, so local archive names do not collide with the
   published `v1.1.7` sidecar.
3. Update `DOI_DEPOSIT_INSTRUCTIONS.md` so `_release/` is safe only when its
   hash matches the release sidecar for the named tag; otherwise instruct
   maintainers to use the downloaded release asset.
4. Refresh `paper_tex/acmart.cls` or document a venue-specific reason for
   retaining v2.03 plus red review line numbers.
5. Either inspect SILLM4Rec full text directly or keep the paper's exclusion
   explicitly based on public repo evidence only.
6. Fill cover-letter maintainer fields and, if the reader PDF is distributed,
   remove the near-blank terminal page.

### Open Questions

- Should the next cut be immediate (`v1.1.8-deposit`) because a manifested doc
  changed after `v1.1.7`, or should HEAD be treated as non-submission work until
  the final DOI release?
- Should the builder refuse to write a `v1.1.7` zip when
  `--verify-git v1.1.7-deposit` fails, even if `--verify-git HEAD` passes?
- Is the current TORS target definitely allowed to retain red review line
  numbers at the intended journal?
- Are Office/FIR-breadth per-user sidecars going to be deposited before review,
  or only upon editorial request/acceptance?

### Running Checklist

- [x] Read automation memory state (none existed).
- [x] Located canonical manuscript, TeX, PDFs, release manifest, release docs,
      figures, tables, prereg/result files, and existing cumulative audit.
- [x] Inspected commits since the last visible audit section.
- [x] Ran strict submission rebuild.
- [x] Ran manifest verification against live tree, HEAD git blobs, and
      `v1.1.7-deposit`.
- [x] Verified builder help, check-only, full build, deterministic zip hash, and
      internal archive checksums.
- [x] Checked published GitHub release metadata and downloaded release
      manifest/sidecar.
- [x] Checked Office V1/V3 wording, cover-letter placeholders, page counts, PDF
      hygiene, and rendered visual samples.
- [x] Fact-checked HSTU-BLaIR constants, AR2023 framing, FMLP/BSARec novelty
      boundary, SILLM4Rec public workflow, and ACM/acmart template drift.
- [x] Updated current prioritized rejection-risk list.
- [x] Appended this timestamped audit section.
- [ ] Cut and verify a new deposit tag, or revise branch docs so the old tag is
      not advertised as the current manifest target.
- [ ] Resolve ACM class/line-number, SILLM4Rec full-text, sidecar deposit, and
      cover-letter freeze items.
...
- Workspace: `C:\Users\rayxc\Documents\R`.
- Branch/HEAD: `codex/bestrec-sota-results` /
  `7c6cd53a3751dcc0938f50731ea06a7092884984`.
- Current deposit tag checked: `v1.1.6-deposit` /
  `60b5f414f23b6b58a907b8e552de253986b05800`.
- New commits since the remembered 20:20 audit: `60b5f414` cut v1.1.6 with
  platform-independent manifest hashing, `--verify-git`, and bundle
  cross-checks; `7c6cd53a` added the response. `git diff v1.1.6-deposit..HEAD`
  shows only `RESPONSE_TO_PAPER_REVIEW_AUDIT.md` changed after the deposit tag.
- Working tree before this audit edit: no tracked modifications; known
  untracked files remained `_bestrec_run/impact_program.DONE`,
  `_bestrec_run/smoke_FIRB_IS_seed1.json`,
  `data_raw_proper/cds_vinyl/CDs_and_Vinyl.csv.gz`, and
  `data_raw_proper/industrial_sci/Industrial_and_Scientific.csv.gz`.
- Sources/artifacts inspected this run: `PAPER_REVIEW_AUDIT.md`,
  `PAPER_SUBMISSION.md`, `paper_tex/sections/*`, `paper_tex/PAPER_TORS.pdf`,
  `PAPER_SUBMISSION.pdf`, `paper_tex/acmart.cls`, `paper_tex/BUILD_NOTES.md`,
  `paper_tex/hygiene_scan_output.txt`, `RELEASE_MANIFEST.json`, `README.md`,
  `DOI_DEPOSIT_INSTRUCTIONS.md`, `CANONICAL_SUBMISSION.md`,
  `VENUE_PLAN.md`, `COVER_LETTER_TORS.md`, `PREREG_OFFICE_V3.md`,
  `OFFICE_V3_RESULTS.md`, `FIR_BREADTH_RESULTS.md`,
  `_bestrec_run/adjudicate_office_v3.py`,
  `_bestrec_run/adjudicate_fir_breadth.py`,
  `_bestrec_run/rebuild_hstu_submission.py`,
  `_bestrec_run/update_release_manifest.py`,
  ...
  `_bestrec_run/hstu_tables.json`, the `v1.1.6-deposit` tag, and
  `_release/bestrec_deposit_v1.1.6.zip`.
- Generated current visual samples from `paper_tex/PAPER_TORS.pdf` with
  PyMuPDF into `tmp/pdfs/paper_audit_20260718_2130/`; no manuscript/source file
  was edited in this run other than this audit file.

### Verdict

The prior v1.1.5 manifest/bundle defect is materially repaired in v1.1.6:
the tag, normalized manifest hashes, and bundled payloads now agree under the
new rules. The new reject-level problem is the Office V3 support chain. The
manuscript says Office V3 passed and counts it as a second per-category
comparison, but the live adjudicator now says the campaign is VOID because its
condition-2 hash check still uses raw bytes after the manifest migrated to
LF-normalized text hashes. A strict reviewer does not have to infer the
authors' intent; they can run the adjudicator and quote its VOID verdict.
...
- `git status --short --branch`
  - Before this audit edit: no tracked modifications; four known untracked
    files listed above.
- `git rev-parse HEAD`; `git rev-parse v1.1.6-deposit`;
  `git rev-list --oneline v1.1.6-deposit..HEAD`
  - `HEAD = 7c6cd53a3751dcc0938f50731ea06a7092884984`.
  - `v1.1.6-deposit = 60b5f414f23b6b58a907b8e552de253986b05800`.
  - One post-tag commit: `7c6cd53a`, response only.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify`
  - PASS: `153` files verified, `0` release-asset files not local.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify-git`
  - PASS vs `HEAD`: `128` git-backed entries match.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify-git v1.1.6-deposit`
  - PASS vs the current deposit tag: `128` git-backed entries match.
- `uv --project _bestrec_run run python _bestrec_run/update_release_manifest.py --verify-git de04d9dcab30d147afa0c8329d67dcb3729fcc87`
  - FAILS with `2` mismatches: `submission_docs/CANONICAL_SUBMISSION.md` and
    `submission_docs/_bestrec_run/update_release_manifest.py`. This is the
    documented parent-commit convention, but it remains a confusing
    provenance field because `git_commit` does not identify the matching tree.
- Independent zip audit for `_release/bestrec_deposit_v1.1.6.zip`
  - Outer SHA256:
    `f2f5d3bc75bf09415721112015178a216c8a94d358a1e918e88952908c3795e4`.
  - Sidecar `.sha256`: matches.
  - Zip entries: `66`; internal `SHA256SUMS.txt` rows: `65/65` OK, `0` bad,
    `0` missing, `0` extra payloads not in sums.
  - `RELEASE_MANIFEST.json` vs bundled payloads: `28/28` checkable
    manifest-listed payloads match under the normalized text hashing rule.
  - `8` git-backed manifest files are not bundled; this is now documented as
    repository-superset evidence rather than bundle payload evidence.
- `uv --project _bestrec_run run python _bestrec_run/build_hstu_tables.py --submission --tables-out $env:TEMP\hstu_tables_submission_audit_20260718_2130.json`
  - PASS: `168` cells recomputed; `149` exact, `19` within-rounding,
    `0` MISMATCH, `0` UNTRACEABLE, all `14` declared claim families sourced.
- `uv --project _bestrec_run run python _bestrec_run/rebuild_hstu_submission.py --strict`
  - PASS: HSTU parity, strict table build, release-manifest verification, MI V2
    gate, and older Office descriptive/VOID check. This did not run Office V3.
- `uv --project _bestrec_run run python _bestrec_run/test_hstu_parity.py`
  - PASS: asserted HSTU stages max diff `0.000e+00`.
- `uv --project _bestrec_run run python _bestrec_run/adjudicate_office_v3.py --no-append`
  - Fresh block `9b1147c62781` at `2026-07-18 21:25:37`.
  - Numeric arms PASS: k16 mean `0.03047`, sd `0.00011`, CI-LB `0.03033`;
    k8 mean `0.03029`, sd `0.00005`, CI-LB `0.03024`; all ten seeds above
    both `0.0279` and `0.0271`.
  - Condition 2 FAILS under the adjudicator's raw SHA256 check for all five
    reference files:
    `_bestrec_run/theirs_runs/office_hstu_blair/hstu-sampled-softmax-n512-blair.gin`,
    `metrics.jsonl`, `run_meta.json`, `tb_logdir_intended.txt`, and
    `_bestrec_run/theirs_runs/office_hstu_blair.log`.
  - Campaign verdict: `VOID` because comparability conditions are violated.
- Manual raw-vs-normalized hash check for the five Office reference files:
  - For every file above, raw SHA256 differs from `RELEASE_MANIFEST.json`, but
    LF-normalized SHA256 matches the manifest exactly. This pins the cause to
    stale adjudicator hash semantics after the v1.1.6 manifest migration.
- `uv --project _bestrec_run run python _bestrec_run/adjudicate_fir_breadth.py --no-append`
  - CONFIRMED for both categories. Industrial_and_Scientific mean `+0.00240`,
    CI `[+0.00183,+0.00297]`; CDs_and_Vinyl mean `+0.00566`, CI
    `[+0.00493,+0.00639]`; both `5/5` positive.
- `uv --project _bestrec_run run python paper_tex\scan_pdf.py paper_tex\PAPER_TORS.pdf`
  - PASS: `40` pages, `0` placeholder/forbidden failures, `20`
    informational SOTA/negation hits.
- PDF page-count check with `pypdf`
  - `paper_tex/PAPER_TORS.pdf`: `40` pages, `612 x 792 pt`.
  - `PAPER_SUBMISSION.pdf`: `46` pages, `612 x 792 pt`.
- Visual render with PyMuPDF
  - Rendered pages 1, 2, 3, 27, 28, 39, and 40 at 1.5x. No obvious clipping,
    unreadable glyphs, broken tables, or margin cutoffs observed in inspected
    samples. Red line numbers are visible throughout the review PDF.
  - Poppler wrapper commands `pdfinfo`/`pdftoppm` failed in this shell with
    "The system cannot find the path specified"; PyMuPDF was used instead.

### External Fact-Check / Novelty And Venue Notes

- ACM's current submissions page says review manuscripts should be
  single-column and use the Primary Article Template LaTeX package `2.16`
  with `\documentclass[manuscript]{acmart}`. Source:
  https://www.acm.org/publications/authors/submissions
- CTAN currently lists production `acmart` version `2.19` dated
  `2026-06-27`, while the vendored local class is v2.03 from `2024/02/04`.
  Source: https://ctan.org/tex-archive/macros/latex/contrib/acmart
- HSTU-BLaIR remains a close comparator rather than background-only prior art:
  its paper evaluates Video Games, Office Products, and Musical Instruments
  from Amazon Reviews 2023 and emphasizes Office sparsity and HSTU-BLaIR gains.
  Source: https://arxiv.org/html/2504.10545v3
- SILLM4Rec remains plausibly non-interchangeable with this paper's full-catalog
  LLOO setting, but it is still close enough to inspect carefully: the public
  repo asks users to download AR2023 5-core data, create candidate product
  ranking tasks, and generate SFT/DPO data. Source:
  https://github.com/MKC-Lab/SILLM4Rec . ACM metadata for the paper identifies
  experiments on three 5-core Amazon Reviews 2023 sub-datasets. Source:
  https://dl.acm.org/doi/full/10.1145/3743093.3771011

### Confirmed Problems

1. **Office V3 is currently not supported by its live adjudicator.** The paper
   counts Office V3 as PASSED, and `OFFICE_V3_RESULTS.md` still records a PASS
   block from `2026-07-13`, but the current script now produces a VOID verdict
   under condition 2.
2. **The Office V3 failure is caused by inconsistent hash semantics across
   tools.** `RELEASE_MANIFEST.json` and `update_release_manifest.py` now use
   LF-normalized hashes for git-backed text files. `adjudicate_office_v3.py`
   still compares raw worktree bytes. On this Windows checkout, all five
   Office reference artifacts raw-hash drift but normalized-hash match.
3. **The strict submission rebuild does not protect the counted Office V3
   claim.** It passes after the V3 adjudicator voids the campaign. For a paper
   whose lead contribution is fail-closed artifact gating, this gap is
   reviewer-visible and serious.
4. **The manifest `git_commit` field remains semantically risky.** The new
   `--verify-git` evidence passes for `HEAD`/`v1.1.6-deposit`, but not for the
   literal commit stored in the manifest. Documentation explains the parent
   convention, but tools and reviewers naturally treat `git_commit` as the
   tree to verify.
5. **Venue-template refresh is still pending.** The paper uses a 2024-vintage
   vendored ACM class while ACM/CTAN current guidance has moved. This may be
   acceptable only with an explicit portal-tested freeze decision.

### Confirmed Fixes / Non-Problems Since The Prior Section

- The v1.1.6 bundle repairs the prior v1.1.5 manifest-vs-payload mismatch:
  internal zip sums, sidecar hash, tag-vs-manifest hashes, and bundled
  manifest-listed payloads are now consistent.
- The strict table emitter path remains hardened: `hstu_tables.json` is
  submission-mode, the strict build recomputes all printed empirical cells, and
  there are no paper mismatches or untraceable cells.
- FIR-breadth remains mechanically confirmed under the frozen internal paired
  filter-vs-no-filter wording.
- The TORS PDF hygiene scan passes and sampled visual renders are legible.

### Plausible Risks / Items Requiring Author Verification

- Does the Office V3 pre-registration's phrase "hash-manifested" legally mean
  raw worktree bytes, or the current manifest's normalized text-hash rule? If
  the latter, the adjudicator must be patched and the erratum should state the
  migration; if the former, v1.1.6's normalized manifest cannot be used as the
  V3 reference condition without a preregistration erratum.
- Should `rebuild_hstu_submission.py --strict` run all counted live
  adjudicators (`adjudicate_office_v3.py` and `adjudicate_fir_breadth.py`) and
  fail unless each counted campaign returns PASS/CONFIRMED?
- Should `RELEASE_MANIFEST.json` gain a separate `manifest_tree_commit` or
  `deposit_tag_commit` field so reviewers have a literal commit to pass to
  `--verify-git`, instead of overloading `git_commit` with a parent-commit
  convention?
- Are red line numbers required by the intended TORS review portal, or should a
  line-number-free final review PDF be generated for upload?

### Concrete Fixes To Make Next

1. Patch `_bestrec_run/adjudicate_office_v3.py` to use the same hash routine as
   `update_release_manifest.py` for git-backed text reference files (or record
   and check both raw and normalized digests explicitly). Re-run it and append a
   fresh adjudication block; do not submit while the live verdict is VOID.
2. Extend `_bestrec_run/rebuild_hstu_submission.py --strict` so it runs the
   Office V3 and FIR-breadth adjudicators and fails on non-PASS/non-CONFIRMED
   verdicts for counted claims.
3. Update `PREREG_OFFICE_V3.md`, `OFFICE_V3_RESULTS.md`, and Section 5.2 only
   after item 1 is fixed, stating precisely which hash rule verifies condition
   2 after the v1.1.6 manifest migration.
4. Replace or supplement `git_commit` with an unambiguous tree/tag field and
   make a documented command such as `update_release_manifest.py --verify-git
   <field>` pass without reviewer interpretation.
5. Finish the ACM template decision: refresh the vendored class or document a
   portal-tested reason to retain v2.03; decide whether line numbers belong in
   the upload artifact.
6. Inspect the SILLM4Rec ACM PDF directly before freeze, or make the exclusion
   rely only on the public repository workflow with explicit candidate-ranking
   / SFT-DPO wording.

### Open Questions

- Is Office V3 intended to remain a counted claim after the v1.1.6 hash-rule
  migration, or should it be temporarily downgraded until the adjudicator is
  repaired?
- Should the next deposit be `v1.1.7-deposit` after the Office V3 adjudicator
  fix, since v1.1.6 currently archives a script that can void a counted claim
  on Windows?
- Should local-only Office/FIR per-user sidecars be deposited now, or is
  aggregate-cell recomputation the declared reproducibility boundary?

### Running Checklist

- [x] Read automation memory and prior cumulative audit.
- [x] Locate canonical manuscript, TeX, PDFs, result files, preregistrations,
      release files, and audit artifacts.
- [x] Verify new commits and current deposit boundary.
- [x] Re-run release-manifest live and git-blob verification.
- [x] Independently audit the v1.1.6 zip, sidecar, internal sums, and
      manifest-listed bundled payloads.
- [x] Re-run strict table build and full strict rebuild.
- [x] Re-run Office V3 adjudicator and isolate the raw-vs-normalized hash
      contradiction.
- [x] Re-run FIR-breadth adjudicator.
- [x] Check PDF page counts, hygiene scan, and representative visual renders.
- [x] Fact-check current ACM template drift, HSTU-BLaIR comparator proximity,
      and SILLM4Rec workflow evidence.
- [x] Update current prioritized rejection-risk list.
- [x] Append this timestamped audit section.
- [ ] Repair Office V3 adjudicator hash semantics and append a fresh PASS/VOID
      block.
- [ ] Add Office V3/FIR-breadth adjudicator verdicts to the strict submission
      gate.
- [ ] Clarify `RELEASE_MANIFEST.json` commit/tag boundary.
- [ ] Finish ACM template and line-number freeze decision.
- [ ] Inspect SILLM4Rec full text or keep exclusion strictly repo-evidence-based.

## Superseded Prior Rejection-Risk List - 2026-07-18 20:20 Snapshot
