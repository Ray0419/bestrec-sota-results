...
It runs fail-closed: bitwise HSTU core-block parity; the strict {N_CELLS}-cell,
{N_FAMILIES}-family artifact graph; release-manifest verification; the governed MI,
...
def _deposit_readme():
    with open(os.path.join(ROOT, "_bestrec_run", "hstu_results_manifest.json"),
              encoding="utf-8") as f:
        graph = json.load(f)
    n_cells = sum(c.get("status") == "OK" for c in graph.get("cells", []))
    n_families = len(graph.get("required_families", []))
    return README_TMPL.format(VERSION=VERSION, DATE=_manifest_date(),
                              N_CELLS=n_cells, N_FAMILIES=n_families)
...
def consistency_gate(candidate=False):
    ...
    graph_count_patterns = {
        "README.md": r"all\s+(\d+)\s+artifact-gated cells across\s+(\d+)\s+families",
        "CITATION.cff": r"recomputes\s+(\d+)\s+active paper cells across\s+(\d+)\s+families",
        ".zenodo.json": r"recomputes\s+(\d+)\s+active paper cells across\s+(\d+)\s+families",
    }
    for name, pattern in graph_count_patterns.items():
        body = metadata[name]
        match = _re.search(pattern, body, _re.IGNORECASE)
        observed = tuple(map(int, match.groups())) if match else None
        if observed != (n_cells, n_families):
            fails.append(f"{name} graph-count statement {observed} != live "
                         f"{n_cells}/{n_families}")
    ...
    if f"requires a {n_entries}-entry candidate bundle" not in readme:
        fails.append(f"README.md candidate inventory statement != {n_entries} entries")
    ...
    if mc != head:
        ...
        if vg.returncode != 0:
            ...
            tc = tag_result.stdout.strip()
            if tag_result.returncode != 0 or not tc:
                detail = tag_result.stderr.strip().splitlines()
                suffix = ": " + detail[-1] if detail else ""
                fails.append("REBUILD mode requires declared tag %s, but git cannot "
                             "resolve it%s -- create the final tag only after metadata, "
                             "clean-clone, and candidate checks pass" % (tag, suffix))
            elif tc != head:
    ...
    README = _deposit_readme()
    expected_graph_phrase = f"strict {n_cells}-cell,\n{n_families}-family artifact graph"
    if expected_graph_phrase not in README:
        fails.append("README template graph counts are stale against the active graph")
