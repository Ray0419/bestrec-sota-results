...
def render_table(key, cfg, caption_md, table_md, footer_md=None, prov_note=""):
    ...
    out.append(r"\par\medskip\begingroup%s\setlength{\tabcolsep}{%dpt}\renewcommand{\arraystretch}{1.12}" % (size, tabcolsep))
    if caption_md:
...
REGISTRY = [
    ...
    dict(key="table1b", fp="method", out="table1b.tex",
         ...
         family="table1b", extra_families=["table1"],  # "Comparison to ours" column cites the table1 headline
    ...
    for cfg, blk in zip(REGISTRY, blocks):
        ...
        if fam_name == "__manifest_theirs__":
            ...
            for xf in cfg.get("extra_families", []):
                fam_text += "\n" + tj["tables"][xf]
            fam_src = "hstu_tables.json:" + ",".join([fam_name] + cfg.get("extra_families", []))
        else:
