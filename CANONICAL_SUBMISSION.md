...
3. **E-G (PREREG_COLDFUSE_V1, verdicts W-C-POS on all five categories):** frequency-binned history-centroid text fusion changed tail-bin test NDCG@10 (train frequency ≤5) by +0.00217 (MI), +0.00243 (IS), +0.00337 (VG), +0.00114 (Office), +0.00481 (CDs); Holm(5); overall within the −0.0005 no-material-cost margin everywhere; SPARSE-WARM REDISTRIBUTION, not cold start (frequency-0 targets never moved; mid/head means negative, exploratory); all selections at the grid ceiling (unbracketed); exact sign-test floor p=.0625 disclosed; Gate-5 literal/normalized dual verdicts per GATE5_CONFORMANCE_DECISION.md. Artifacts: results_*_COLDFUSE_*, coldfuse_v1_adjudication.json.
