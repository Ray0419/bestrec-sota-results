...
| # | Required fix | Done |
|---|---|---|
| 1 | Update `CANONICAL_SUBMISSION.md` | ✅ rewritten: MI = the only counted per-category comparator win; Office descriptive/VOID with the anomaly explained and VOID retained; FIR ablations complete; the fail-closed gate is `--submission` (163 cells / 0 / 0 / 12 families) *(count as of this response's date; grows as evidence lands — the strict build output is authoritative)*; `results_OFFICE_*` and `results_FIRABL_*` present, nothing "pending"/"in flight"; theirs-on-ours artifacts and both releases enumerated |
