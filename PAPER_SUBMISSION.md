...
We evaluate a small, detachable causal finite-impulse-response (FIR) residual for sequential recommendation on Amazon Reviews 2023 (5-core, full-catalog, leave-last-out). Outcome-known internal estimates are positive on Musical_Instruments (+0.002265, ordinary Welch 95% CI [0.001928, 0.002602]) and, under one untuned configuration with matched initialization, on Industrial_and_Scientific (+0.002110, ordinary paired CI [0.001820, 0.002399]) and CDs_and_Vinyl (+0.006150 [0.005849, 0.006450]). A subsequently frozen prospective Software attempt, with validation-only checkpoint selection and one sealed TEST evaluation per checkpoint, found learned FIR−identity **+0.005062, ordinary paired 95% CI [+0.004591, +0.005533]** (8/8 positive; two-sided p=3.75×10⁻⁸), clearing its pre-declared +0.000500 practical threshold. This is same-investigator, same-code-lineage, same-Amazon-family evidence under local same-user custody—not independent confirmation. A pre-declared but outcome-known six-arm control study found that learned taps did not separate from a shared causal filter (−0.000081 [−0.000337, 0.000175]) or parameter-matched nonlinear causal control (+0.000204 [−0.000038, 0.000446]). A subsequent equal-parameter current-position-only DCT/GELU placebo changed identity by −0.000069 [−0.000200, +0.000061], whereas learned FIR exceeded that placebo by +0.001941 [+0.001788, +0.002095] in the frozen three-test Holm family. This discriminates learned FIR from that tested compound non-temporal residual; because basis/rank, activation, channel mixing, and temporal access change together, it does not isolate temporal access or establish per-channel-tap necessity or generalization. Secondary findings are two environment-caveated per-category point-estimate threshold comparisons and a Musical_Instruments frequency-5-heavy text-tail effect; neither licenses a general system-ranking or cross-dataset claim. A fail-closed graph recomputes 196 paper-bound cells across 20 families and preserves null, deviated, and VOID outcomes alongside positive results.
...
Recommender evaluation is vulnerable to mistuned baselines, incompatible protocols, and irreproducible result selection (Ferrari Dacrema et al., 2019; Ferrari Dacrema et al., 2021). We therefore pair the method with a per-paper discipline: git-frozen protocols, a fail-closed graph that recomputes 196 reported cells across 20 claim families, environment-caveated comparator regeneration, and symmetric adjudication. Failures are retained. In particular, the original Office_Products protocol remains VOID after its own floor check exposed an environment mismatch, although a redesigned fresh-seed protocol later passed (§5.2; Appendix A.0).
...
2. **Internal evidence, prospective same-user validation, and a mechanism boundary.** Learned FIR–identity estimates are positive on three outcome-known category/split settings (§5.2). A separately frozen Software attempt then produced +0.005062 [+0.004591, +0.005533], clearing its pre-declared +0.000500 practical threshold with validation-only selection and sealed final evaluation. It is prospective, but remains same-investigator, same-code-lineage, same-Amazon-family evidence under local same-user custody—not independent confirmation. On Musical_Instruments, learned taps do not separate from a shared causal filter, but they beat an equal-parameter current-position-only DCT/GELU placebo by +0.001941 [+0.001788, +0.002095], while the placebo–identity interval spans zero. This discriminates the learned FIR arm from that compound placebo; it does not isolate temporal access, establish unique necessity of per-channel taps, or show cross-domain generalization. A frequency-5-heavy text-tail effect is secondary and does not establish cross-dataset heterogeneity (§5.3–§5.4).
...
- The two added components — the **strictly causal FIR temporal filter** (positive internal identity contrasts on three outcome-visible categories plus a positive prospective same-user Software contrast; learned FIR separates from one equal-parameter compound non-temporal placebo in MI, while the shared causal control prevents per-channel-tap attribution; §5.2) and **TAPE** — plus the **MI frequency-5 tail case** (§5.3) and its **interaction-thinning titration** level-contrast result (§5.4). The novelty boundary for each is stated explicitly in Table 0 (§2.3).
...
|---|---|---|---|
| HSTU base | Zhai et al., 2024: pointwise `silu(QKᵀ + rab) V`, per-block normalization, and relative biases | pure-PyTorch implementation; two spurious normalizations removed; core-block parity only at a mirrored aligned configuration; no pinned end-to-end reproduction (§3.2, §5.6, §6.3) | Table 1 ablations; prior-work implementation |
| BLaIR / SBERT text | Hou et al., 2024; Reimers & Gurevych, 2019; Wang et al., 2020: frozen encoders and Hou et al.'s MLP adaptor | no method change; frozen off-the-shelf features | Table 1a encoder comparison; prior work |
| Label smoothing / time bias | Szegedy et al., 2016; TiSASRec; HSTU (Zhai et al., 2024) | integration with full-catalog chunked-softmax and this backbone | +0.0013 label-smoothing and +0.0027 time-bias single flags (Table 1); prior work |
| FMLP-Rec / BSARec / FreqRec / WEARec filters | Zhou et al., 2022; Shin et al., 2024; He et al., 2026; Xu et al., 2026: sequence filtering, softmax-attention low-pass framing, inter-session spectral paths, and adaptive wavelet/frequency filtering | none inserted as-is: FMLP-Rec/BSARec filters are bidirectional; FreqRec/WEARec are larger systems | motivation and boundary (§3.7b); prior work |
| Causal FIR adaptation (ours) | the filtering line above; causal convolutional encoders such as Caser and NextItNet are prior art | minimal leak-free left-causal depthwise residual, Δ=0 identity initialization yet gradient-active, before an HSTU-style all-position stack | outcome-known FIR−identity estimates: MI +0.002265, IS +0.002110, CDs +0.006150; prospective same-user Software estimate +0.005062 [+0.004591,+0.005533]; learned FIR beats an equal-parameter current-position-only placebo, but not the shared causal filter (§5.2); **incremental modular contribution, not independent confirmation** |
| TAPE (ours) | TIGER (Rajput et al., 2023), VQ-Rec, ProtoMF; prototype/semantic-ID motivation and frozen text | frozen soft assignments gate a zero-init additive prototype table | +0.0009 single flag; +0.0004 four-seed check; supporting ablation (§5.1) |
| Tail / thinning analysis (ours) | standard popularity strata; MELT; DropoutNet; CLCRec | per-dataset text−ID tail contrast and matched-R1 interaction/user thinning | MI +0.000420 (outcome-visible, not confirmatory); VG null; cross-dataset p=.13; user-mode p=.058; **secondary empirical boundary** (§5.3–§5.4) |

**Closest filtering systems.** FMLP-Rec and BSARec establish learnable sequence filtering and frequency-rescaled attention; BSARec's Theorem 1 concerns repeatedly applied softmax attention, not our HSTU-style operator. C3SASR and AdaMCT already combine causal/local convolutions with attention. Current systems are broader still: FreqRec (He et al., 2026) couples inter- and intra-session spectral paths with a frequency-consistency loss, while WEARec (Xu et al., 2026) uses sequence-adaptive frequency filters and wavelet enhancement. Our distinction is therefore only the small gradient-active, identity-initialized **left-causal depthwise FIR residual** under one HSTU-style all-position, full-catalog protocol—not convolution, filtering, frequency modeling, or temporal specificity as general contributions.
...
**Prospective Software validation (`PREREG_FIR_PROSPECTIVE_SW_V3.md`; frozen before launch; eight matched seed blocks; sealed one-shot final TEST; verdict `SW-V3-PRACTICAL-POS`).** The learned and identity arms used exact per-seed matched backbone initialization. Training loaded TEST rows only for the explicitly pre-declared transductive item catalog and emitted no TEST scores; checkpoints were selected solely by validation NDCG@10. After all 16 checkpoints existed, the committed driver created immutable READY/evaluation seals, evaluated each selected checkpoint once on TEST, and immediately invoked the committed adjudicator as first endpoint reader. Mean TEST NDCG@10 was **0.120200 learned versus 0.115138 identity**; learned minus identity was **+0.005062, ordinary paired 95% CI [+0.004591, +0.005533]**, paired t(7)=25.39, two-sided p=3.75×10⁻⁸, with 8/8 differences positive. The CI lower bound cleared the frozen practical threshold +0.000500, yielding the exact verdict `SW-V3-PRACTICAL-POS`. This closes the paper's prospective-evidence gap, but not its independence gap: it is a same-investigator, same-code-lineage, same-Amazon-family attempt with local same-user operational handoff and no external escrow or independent custody. It is therefore not independent confirmation or cross-domain replication.
...
The main result is deliberately modular and narrowly identified. Positive FIR–identity contrasts on three outcome-visible categories are joined by a positive, practically thresholded prospective Software contrast. Together they show that the tested trainable left-causal residual arms improve this frozen-identity backbone in these settings. The prospective attempt improves the evidence chronology, but its same investigator, code lineage, Amazon family, and local same-user custody mean it is not independent confirmation. An equal-parameter current-position-only DCT/GELU placebo does not show a detected improvement over identity, and learned FIR exceeds it under matched initialization. This discriminates learned FIR from that compound residual but does not isolate temporal access because basis/rank, activation, channel mixing, and temporal access change together. The shared causal filter is also not separated from learned per-channel taps. The evidence therefore supports neither uniquely learned FIR coefficients, a universal frequency-filter benefit, nor a new end-to-end architecture.
...
- **Historical, canonical, control, and prospective evidence.** Same-numbered historical arms were not initialization-paired, so their paired interpretations are withdrawn. E-A used matched initialization but its frozen registered interval is Welch/Satterthwaite (the paired difference is descriptive); canonical breadth and active controls use ordinary paired intervals. Those studies reused outcome-visible splits; the breadth categories were selected after favorable package outcomes and TEST was evaluated each epoch. The six-arm final evaluation was sealed, but that campaign ran across a dirty evolving tree and its sidecars lacked independent custody because the ignore rule covered the wrong suffix. Fixed MA/HP are algebraically redundant and shared/nonlinear arms prevent a learned-tap-specific claim. The later pointwise-placebo protocol was precommitted and source-hash-bound but executed from a tracked-dirty tree; it adds only a learned-FIR-versus-compound-current-only contrast on outcome-known MI, not temporal isolation or per-channel necessity. Software V3 supplies a prospective practical-effect cutoff with validation-only selection and sealed final evaluation, but it remains same-investigator, same-code-lineage, same-Amazon-family evidence under local same-user custody—not independent confirmation. Office V1 remains **VOID**; Office V3 supports only its frozen point-estimate wording.
...
A separately frozen prospective Software attempt found learned FIR−identity
+0.005062 [+0.004591, +0.005533], with 8/8 positive paired differences and the
ordinary 95% CI lower bound above its pre-declared +0.000500 practical threshold.
Because it remains same-investigator, same-code-lineage, same-Amazon-family work under
local same-user custody, it is prospective validation but not independent confirmation.
...
The evidence supports neither unique necessity of learned
per-channel FIR coefficients, external-comparator superiority, nor independent
...
`rebuild_hstu_submission.py --strict` then checks HSTU parity, recomputes the 196
...
release manifest. Table 0's literature/novelty ledger is citation-checked prose, not a
...
- Hu, G., Zhang, A., Liu, S., Cai, Z., Yang, X., Wang, X., 2025. AlphaFuse: Learn ID Embeddings for Sequential Recommendation in Null Space of Language Embeddings. SIGIR. doi:10.1145/3726302.3729894. *(closest frozen-text+ID comparator; current port is governed noncountable evidence — §2.3)*
