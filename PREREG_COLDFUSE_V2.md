# ⛔ EXPOSED_PROTOCOL_DEVIATED — ERRATUM (audit 2026-07-23 22:00 / 2026-07-24 03:59)

**This campaign (E-G2) is EXPOSED and can NEVER confer counted status.** On 2026-07-23 a `git add -A` (commit df5afc9f) swept 54 COLDFUSE2 artifacts into git -- including 14 confirm JSON/NPZ PAIRS (28 confirmation artifacts) plus 14 base + 12 fusion files and pushed them to the public branch before any adjudication (which never occurred; E-G2 stayed descriptive) — the exact no-interim exposure this protocol existed to prevent. Every "first reader" / "can grant counted status" / W2-confirmatory statement below is RESCINDED. The immutable launch blob/commit is preserved unchanged for provenance; this erratum sits above it. Any later summary of E-G2 is DESCRIPTIVE-ONLY and carries this classification; the adjudicator's confirmatory (W2-POS/…) exit is disabled. The sole remaining path to a counted sparse-warm result is a future **E-G3** with (a) a genuinely untouched holdout behind a separate custodian, (b) endpoints stored OUTSIDE git until a sealed one-time adjudication (hooks/CI, not `.gitignore` alone), (c) the 8 hardened adjudicator gates, (d) all-eight-seed structural-null retention, and (e) a pinned frozen environment.
