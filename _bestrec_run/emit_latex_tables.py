...
def write_lf(path, text):
    """Write deterministic LF bytes even when upstream tools emit CRLF."""
    text = text.replace("\r\n", "\n").replace("\r", "\n")
...
def main():
    ...
    for cfg, blk in zip(REGISTRY, blocks):
        ...
        write_lf(os.path.join(OUTDIR, cfg["out"]), tex)
    ...
    write_lf(os.path.join(OUTDIR, "TABLES_PROVENANCE.json"),
