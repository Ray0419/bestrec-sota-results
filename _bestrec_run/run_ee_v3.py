#!/usr/bin/env python3
"""Train one fresh E-E V3 arm/seed with masked-VALID selection and no TEST read."""
...
def valid_terminal(arm: str, seed: int) -> bool:
    ...
    if not checkpoint.is_file():
        raise RuntimeError("partial terminal E-E V3 training bundle")
...
def main() -> int:
    ...
    if valid_terminal(arm, seed):
        print(f"E-E V3 terminal bundle already valid: {arm}/{seed}")
        return 0
    ...
    if device.type != "cuda":
        raise RuntimeError("PREREG_EE_V3 requires CUDA")
    ...
    if not started.exists():
        ...
        if (prior.get("protocol") != common.PROTOCOL or prior.get("arm") != arm
            ...
            raise RuntimeError("E-E V3 STARTED record drift")
    ...
    if latest.exists():
        ...
        if (state.get("protocol") != common.PROTOCOL or state.get("arm") != arm
            ...
            raise RuntimeError("E-E V3 resume checkpoint drift")
        ...
        print(f"E-E V3 resume {arm}/{seed} at epoch {start_epoch}", flush=True)
    ...
    for epoch in range(start_epoch, common.MAX_EPOCHS):
        ...
        for batch in loader:
            ...
            if not torch.isfinite(loss):
                raise RuntimeError(f"non-finite E-E V3 loss at epoch {epoch}")
            loss.backward()
            optimizer.step()
    ...
    if best_epoch < 0 or best_metrics is None or not common.best_path(arm, seed).is_file():
        raise RuntimeError("E-E V3 training produced no valid checkpoint")
    ...
    print(f"E-E V3 TRAINING COMPLETE TEST UNREAD: {arm}/{seed}", flush=True)
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"E-E V3 TRAINING FAILURE: {exc}", file=sys.stderr, flush=True)
        raise
