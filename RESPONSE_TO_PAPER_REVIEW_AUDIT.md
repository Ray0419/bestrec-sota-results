...
| # | Finding | Action this tick | Where |
|---|---|---|---|
| 1 | Different evaluator/estimand (AlphaFuse ranks UNMASKED; our evaluator masks seen history) — retract "identical estimand / same LLOO / apples-to-apples / fusion-only" | **VERIFIED**: `run_sasrec_sbert.py:1626-1630` masks the full train+val history (`final[k,items]=-inf`); AlphaFuse `utils.evaluate` does not. Masking removes distractors and inflates our arm. `adjudicate_ee.py` HARDENED (estimand-parity + exact-seed + Arm-B gates) -> returns **PILOT_NONCOUNTABLE**; PREREG_EE **ERRATUM E2** retracts the four phrases + adds a top banner; values NOT integrated | ee_adjudication.json; PREREG_EE E2 |
| 2 | Whole systems differ, not only fusion; unequal tuning | Frozen into E-E V2 spec (E2 item 2): matched-backbone AlphaFuse fusion on/off under ONE evaluator with EQUAL val-only budgets; two full systems kept only as SECONDARY descriptive; report params/FLOPs/mem/latency | PREREG_EE E2 |
| 3 | Adjudicator self-check self-destructs on its own "no superiority/SOTA" wording | Already fixed earlier this session (**ERRATUM E1**): substring false-positive on NEGATED disclaimers; disclaimer strings reworded only — verdict rule, computation, numbers unchanged | PREREG_EE E1; git diff |
| 4 | Comparator identity wrong (VG 4th seed is 09 not 19; glob absorbs; AlphaFuse-only can be labelled reportable) | E-E V2 spec (E2 item 3): register exact filenames/seeds/sizes/SHA, require exactly those files, Arm-B mandatory; corrected "09 not 19". Interim: adjudicator now requires exactly seeds {22,23,24} AND Arm-B present or it will not report | PREREG_EE E2; adjudicate_ee.py |
| 5 | Run identity under-specified; no per-user rank sidecars; only unrestricted MRR emitted | E-E V2 spec (E2 item 4): immutable attempt dirs + append-only ledger + atomic writes + per-user rank sidecars + fail-closed identity/schema/finite/range checks. (`metrics_family.py` already reconstructs the full HR/NDCG/MRR family exactly from per-user ranks) | PREREG_EE E2 |
| 6 | Port/env/seal not frozen; E-E endpoints absent from seal patterns | `results_EE_*` added to the seal-hook regex + `.gitignore`; the 3 pilot endpoint JSONs untracked (kept local; HARD RULE = no delete). Also fixed a real seal **pre-commit bug**: it listed deletions (`--name-only`) and thus BLOCKED the re-seal; now `--diff-filter=AM` so untracking a sealed endpoint is allowed. V2 spec pins upstream AlphaFuse SHA + licence + lockfile/container + clean state + hardware + out-of-repo endpoints | seal_patterns.sh; pre-commit; .gitignore |
| G | E-B `bootstrap_pod.sh` still clones/installs/caches for the VOID launcher | Now **HARD-REFUSES at entry** (exit 3) | cloud/bootstrap_pod.sh |

**Disclosed open (forward path, not silently dropped):** E-E **V2 build** (shared exact-rank evaluator + synthetic seen-item/tie tests, matched-backbone factorial, immutable attempts, per-user ranks, pinned port) — the only route to a countable comparison; **LLM2Emb** fair benchmark OR executable exclusion + narrowed novelty; the **WS4 synthesis rewrite** (28k words -> scientific spine + labelled supplement; one forest/stratum figure with per-seed dots + CIs; abstract "every / all public" scope-correction; `PAPER_DRAFT.md` delete-before-submission block; replace "null / refuted" with "estimate crosses zero / non-reproduced"; semantic parity across md/tex/PDF); **E-G3** PREREG/custodian/builder/retrieval/adjudicator; the **deposit is 51 commits stale** (will NOT upload; banner refresh queued); **author/affiliation metadata** (human decision). **Counted set (MI V2, Office V3) unchanged; no claim broadened; E-E currently yields NO countable number.**


## Response — to Audit Run 2026-07-24 09:59 Australia/Sydney (responded 2026-07-24; MACHINE-CHECKED: render CLEAN 61pp, venue build exit 0, closure ledger PASS, strict chain green after commit)

**Verdict accepted, including the parts that rebut my own prior claims.** The 09:59 findings correctly show my last two ticks overstated the seal ("DONE", "would have stopped both exposures") and E-B disablement ("all P0 paths disabled"), and that a live manuscript claim-drift remained. Corrected below; the larger design/editorial items are disclosed as open.

| # | Finding | Action | Verification |
|---|---|---|---|
| 1 | E-G3 custody false (labels locally reconstructible; confirm-writes-metrics vs adjudicator-only-reader contradiction) | E-G3_DESIGN gains a top CORRECTION block: locally-held labels are NOT withholding; a countable successor needs an **independent custodian/eval service** holding non-reconstructible labels + **signed prediction-only** submissions (model side writes predictions, custodian computes metrics) | E-G3_DESIGN.md |
| 2 | Seal is advisory, not a boundary (no CI; fresh clones don't install; `--no-verify`; pre-push net-diff misses add-then-delete; "would have stopped both" false) | **Overclaim downgraded** to "LOCAL advisory speed-bump" in EXPERIMENT_PROGRAM; **added `.github/workflows/seal.yml`** running `ci_seal_check.sh` on every push (server-side, hook-independent) so hook-less clones still fail; the honest limitation (a hook is not a boundary) is stated | program; workflow file |
| 3 | E-B V1 still launchable (cloud/README launch-ready; bootstrap/sharder/run_shard/eval_final_model active; sharder points to old `controls/`) | **cloud/README VOID banner**; `make_shards_eb.py`, `run_shard.py`, `eval_final_model.py` now hard-refuse (exit 3); `bootstrap_pod.sh` refuses; "all P0 disabled" claim retracted (it wasn't) | grep; scripts compile |
| 4 | Temporal-design leakage + estimand ambiguity (full-period k-core uses future; review-time ≠ availability; "item-arrival cold" unsupported) | E-G3_DESIGN correction: causal pre-cut cohort, point-in-time snapshot, forecast horizon + censoring; renamed **"first-observed-interaction cold"** (not item-arrival) absent availability evidence | E-G3_DESIGN.md |
| 5 | Manuscript claim-drift (TeX/PDF: "confirming/refuting", "the refutation is unaffected", "the positive mechanism"; conclusion "refuting") vs canonical "inconclusive" | **Fixed (md+tex, results+conclusion):** "split result … tail is a correlate only, inconclusive (no equivalence margin, not a refuted null)"; "This direction is unaffected"; "a candidate mechanism (hypothesis, not identified)"; conclusion "inconclusive, not-confirming" | §5.4/conclusion; build exit 0 |
| 6 | E-G2 `--descriptive-only` still says "first reader", emits bare W2 verdicts, generic JSON | Adjudicator now prints "DESCRIPTIVE-ONLY — EXPOSED, NON-COUNTABLE; NOT a first reader"; JSON gains a top-level `classification: EXPOSED_PROTOCOL_DEVIATED_NON_COUNTABLE`, `countable: false` | adjudicate_coldfuse_v2.py |
| 7 | Stale chronology ("~59 min before a declared one-time adjudication" — none occurred; "14 pairs") | Corrected across prereg/canonical/program: **54 COLDFUSE2 artifacts incl. 14 confirm JSON/NPZ PAIRS (28 confirmation artifacts) + 14 base + 12 fusion**; completion remained descriptive and UNADJUDICATED | prereg/canonical/program |
| 8 | Deposit STOP banner numbers stale (was 37/161; actually 42 commits/189 mismatches) | Banner refreshed to 42+ commits / ~189 mismatches | DOI_DEPOSIT_INSTRUCTIONS.md |
| 9 | Completeness gate overclaim (omits drivers/DESIGN/workflows; accepts submission_docs; ignores git error; not NUL-safe) | Gate **widened** (run_*/fuse_* drivers, `*_DESIGN.md`, `.github/workflows/**`), **NUL-safe** (`git ls-files -z`), **fail-closed on git error**, protocol_code-only; governed surfaces re-added; "every surface" claim scoped to the enumerated patterns | update_release_manifest.py |
| 10 | E-B control docstring falsely says Philox (it's PCG64); covariance not matched | Docstring corrected to PCG64/default_rng; controls_v2 realized-artifact + covariance/anisotropy-matched null folded into the required E-B V2 spec | make_control_caches.py; PREREG_TEXTPERM_V1 void |

**Deferred (disclosed open, not silently dropped):** #11 (E-G2 remains a descriptive operational record, no number integrated — correct disposition), #12 (E-G1 limited: outcome-visible seed-stability on one exposed split — already so stated), #13 (AlphaFuse/LLM2Emb benchmark-or-exclusion), #14 (the 28k-word → scientific-spine + forensic-supplement rewrite; `PAPER_DRAFT.md` "delete before submission" block removal; R1/R2 legend nonsignificance mislabel), #15 (author/blind PDF metadata, venue tagging, reader outline, near-empty final page), #16 (untouched external holdout / temporal replication / ethics / licensing / accessibility). These are the genuine road to a submittable paper and are design/editorial work for coming ticks. **No claim broadened; counted set (MI V2, Office V3) unchanged; E-G1/E-G2 both exposed/descriptive-only; E-B void; E-G3 design-only.**

## Response — to Audit Run 2026-07-24 03:59 Australia/Sydney (responded 2026-07-24; MACHINE-CHECKED: render CLEAN 61pp, venue build exit 0, closure ledger PASS, strict chain green after commit)

**Verdict accepted.** The 22:00 reclassification was real but hadn't reached the manuscript, E-G2's own prereg/adjudicator, or the void E-B's execution paths. All P0 items are now executed structurally (false status removed and unsafe execution disabled), not just described.

| # | Audit item (03:59, P0) | Action | Verification |
|---|---|---|---|
| P0-1 | Manuscript/draft/TeX still call E-G2 the independent route to confirmation | Replaced: E-G2 "was intended as that independent replication but was itself exposed … confers no counted status either; the only remaining path is a future repository-sequestered E-G3" (md + tex + PDFs rebuilt) | §5.8; build exit 0 |
| P0-2 | PREREG_COLDFUSE_V2 + adjudicator still claim/test confirmatory status | **PREREG_COLDFUSE_V2 bannered** `EXPOSED_PROTOCOL_DEVIATED` (immutable launch blob preserved below it); **`adjudicate_coldfuse_v2.py` confirmatory exit DISABLED** — it now refuses unless `--descriptive-only`, and even then every W2 verdict string carries "DESCRIPTIVE-ONLY; EXPOSED, not counted" | prereg banner; adjudicator compiles; guard returns 3 |
| P0-3 | Void E-B still runnable (program CLOUD-READY; adjudicator pools n=9; orchestrator launches) | Program label → **DISABLED/VOID**; `adjudicate_textperm_v1.py` hard-refuses (returns 3); `cloud/run_all_pod.sh` neutered (exits 3) with the endpoint git-push removed | all three compile/parse; grep |
| P0-4 | `.gitignore` bypassed by `git add -f` endpoint push in the orchestrator | The entire `git add -f … && git push` endpoint block + self-stop-on-push is **removed**; the script now exits 3. A sealed E-B V2 importer (endpoints OUTSIDE git, hooks/CI) is the registered replacement | run_all_pod.sh diff |
| P0-5 | Realized random control caches still un-normalized (~19.6× norm) | The two stale `*__random.npy` caches **deleted**; `make_control_caches.py` now writes to a NEW versioned root `controls_v2/` and **always overwrites** (atomic temp+rename), so the row-normalized random is the only realized artifact | rm confirmed; generator compiles |
| P0-6 | Deposit instructions say "ready to upload" while `v1.1.11-deposit` is 37 commits / 161 mismatches behind | `DOI_DEPOSIT_INSTRUCTIONS.md` **STOP/STALE banner**: do not upload v1.1.11 as the current release; current-tree gate green, named-deposit gate RED; a new tag is cut only after stabilization | deposit banner |
| — | §5.8 test-once wording, "measured/pre-declared/under one governed", "provably does nothing", `but---per` rendering, v2-in-history distinction, README 276→420 | All fixed (md + tex): confirmation evaluator made one final paired pass (upstream trainer/fusion exposures disclosed); "observed, protocol-deviated … on these fixed splits"; "no HR/NDCG@10 change observed for frequency-zero targets"; clean punctuation; v1/v3 current + v2 recoverable-from-history; count corrected | §5.8; README |
| — | protocol_code omits E-G2/E-B governed surfaces | **12 governed surfaces added** to `protocol_code` (PREREG_COLDFUSE_V2, PREREG_TEXTPERM_V1, both adjudicators, fuse_cold_confirm2, run_eg2_parallel, all 6 cloud drivers) | manifest diff; strict verify |

**E-G2 completion (since your cutoff):** the local campaign finished 40/40 confirm files but is **descriptive-only forensic data** — NOT adjudicated into any counted number, NOT integrated (recorded that way last commit). Its endpoint artifacts are gitignored/retained locally; only the pre-exposure 54 committed partials remain in public history as the exposure record.

**Deferred (P1/P2, disclosed as open):** the genuinely-untouched holdout + separate-custodian E-G3 design, hooks/CI enforcement of out-of-repo endpoint storage, the metadata-only sealed-preflight + 8 hardened gates, closest-baseline benchmarking (AlphaFuse/LLM2Emb — LLM2Emb DOI 10.1016/j.eswa.2026.133375 now known; enters at E-G3/E-E with verified authorship), a negative manifest-completeness gate, the §5.8 three-figure/three-table rewrite + forensic supplement, author/blind metadata, and the new immutable deposit tag. **No claim broadened; the counted set (MI V2, Office V3) is unchanged; E-G1 and E-G2 are both exposed/descriptive-only.**
