#!/usr/bin/env python3
"""One-shot sealed TEST evaluator for a READY PREREG_EE_V3 family."""
...
def main() -> int:
    ...
    train = common.validate_training_bundle(arm, seed,
                                            expected_commit=str(ready["repository_commit"]))
    ...
    if device.type != "cuda":
        raise RuntimeError("PREREG_EE_V3 sealed evaluation requires CUDA")
    ...
    if records is None or set(records) != {
        ...
        raise RuntimeError("sealed evaluator did not produce the exact rank sidecar schema")
    ...
    print(f"E-E V3 SEALED TEST COMPLETE (metrics withheld): {arm}/{seed}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"E-E V3 SEALED TEST FAILURE: {exc}", file=sys.stderr, flush=True)
        raise
