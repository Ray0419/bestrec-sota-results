...
| 6 | **Uneven statistical evidence** | Every manifest cell carries `evidence_class: confirmatory / exploratory` (finding-6 labeling, enforced by the artifact graph); the paper's Table-2 caption already states the power inequality; correction families and sample units to be stated per-claim in the final consistency pass. | IN PROGRESS |
