#!/usr/bin/env python3
"""Evaluate the frozen six-cell sparse-support interaction experiment."""
...
def contrast_stats(
    ...
    delta = sum(coefficients[name] * metrics[name] for name in coefficients)
    ...
    return {
        ...
        "users_positive": int((delta > 0).sum()),
        "users_negative": int((delta < 0).sum()),
        "users_zero": int((delta == 0).sum()),
...
def main() -> int:
    ...
    parser.add_argument("--data-name", default="ML-1M")
    parser.add_argument("--full", type=Path, required=True)
    parser.add_argument("--full-sparse", type=Path, required=True)
    ...
    arm_metrics: dict[str, dict[str, dict[str, np.ndarray]]] = {}
    ...
    contrasts = {
        "T_minus_F": {"T": 1.0, "F": -1.0},
        "A_minus_S": {"A": 1.0, "S": -1.0},
        "interaction": {"A": 1.0, "S": -1.0, "T": -1.0, "F": 1.0},
        "A_minus_R": {"A": 1.0, "R": -1.0},
        "A_minus_B": {"A": 1.0, "B": -1.0},
        "B_minus_S": {"B": 1.0, "S": -1.0},
        "B_minus_R": {"B": 1.0, "R": -1.0},
        "R_minus_S": {"R": 1.0, "S": -1.0},
    }
    result = {
    ...
    for mask_name in masks:
        ...
        for metric_name in metrics:
            ...
            mask_result["contrasts"][metric_name] = {
                contrast_name: contrast_stats(
                    by_arm, coefficients, args.seed, args.bootstrap_samples
                )
